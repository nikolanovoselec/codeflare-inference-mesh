...
3. Only a completed run by an admin or automation caller updates the stored summary; a read-only viewer's run, and any run that hit its deadline, return the measurement leaving the stored summary unchanged. <!-- @impl: packages/router-worker/src/speed-test.ts::runSpeedTest --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-034 REQ-SEC-010 measures a read-only viewer speed test without overwriting the stored summary) --> <!-- @test: packages/router-worker/src/speed-test.test.ts (REQ-ADM-034 returns a bounded partial measurement when the upstream stops sending) -->
