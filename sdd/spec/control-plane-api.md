...
6. A malformed JSON request body on an `/api/v1` endpoint is rejected with `400` `invalid_json`, never a `500`. <!-- @impl: packages/router-worker/src/router.ts::createRouter --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-API-002 rejects a malformed JSON body with 400 invalid_json) -->
