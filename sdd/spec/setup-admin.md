...
5. The direct playground endpoint forwards the internal model through the router's direct scheduling path, bypassing the gateway and honoring the direct-session affinity contract. <!-- @impl: packages/router-worker/src/router.ts::handlePlaygroundDirect --> <!-- @impl: packages/router-worker/src/router.ts::runInference --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-029 playground direct target selects a node and forwards the internal model straight to it) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-029 REQ-RUN-011 direct playground sends a stable session user for affinity) -->
