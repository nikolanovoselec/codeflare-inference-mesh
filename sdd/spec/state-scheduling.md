...
5. The Worker translates the absence of a ready node into `503 no_healthy_node` with a request ID. <!-- @impl: packages/router-worker/src/inference.ts::runInference --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-005 returns 503 no_healthy_node when no eligible node is ready) -->
6. A Mesh transport failure reaching the selected node surfaces as `502 node_unreachable` with a request ID. <!-- @impl: packages/router-worker/src/inference.ts::runInference --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RTR-002 REQ-SCH-005 returns 502 node_unreachable when the mesh fetch throws) -->
