...
The console renders `GET /admin/status` as a live Codeflare operator surface: a reduced-motion-safe Codeflare wordmark, a six-tile hero status strip (available nodes, consumed/total VRAM, live fleet throughput summed from per-machine heartbeat tok/s, the mesh count with how many meshes are serving, custom-domain status, and the fleet's desired agent version), runtime binary install status, derived work state, a hub-and-spoke mesh topology with a mesh selector that filters the canvas to one machine group plus node and model detail drawers, and a sortable nodes table. ([REQ-OBS-010](../../sdd/spec/observability.md#req-obs-010-live-throughput-surface)) ([REQ-OBS-012](../../sdd/spec/observability.md#req-obs-012-runtime-status-detail-surface)) ([REQ-ADM-015](../../sdd/spec/setup-admin.md#req-adm-015-mesh-visualization)) ([REQ-ADM-039](../../sdd/spec/setup-admin.md#req-adm-039-overview-mesh-status-cards)) ([REQ-ADM-033](../../sdd/spec/setup-admin.md#req-adm-033-runtime-binary-version-and-install-visibility)) ([REQ-ADM-036](../../sdd/spec/setup-admin.md#req-adm-036-operator-dashboard-visual-identity))

The Overview's "Mesh status" panel renders a responsive grid of per-mesh dashboard cards, each edged in its status tone (the edge alone carries state — no status word renders): the mesh name paired with its callable route chip, the active model's display name over its mono model file reference and a row of its provider and serving-mode pills (or a no-model note), a machines/serving metric strip whose third cell shows the model's last stored Speed Test as prompt/generation tok/s, and a serving-capacity track filled to the served fraction.

A split model's card also carries its split state — `data-split-state` reads `split` with a formed two-plus-stage topology and `fallback` (warn-toned) when machines serve without one — and a `data-mesh-note` line surfaces the most actionable degradation detail: a node runtime error verbatim, the single-node-fallback notice, or the split readiness verdict. A machine counts as serving only when its catalog claim is corroborated by a ready/running runtime or a stage assignment, so an api-client advertising the mesh catalog never reads as Serving. ([REQ-ADM-039](../../sdd/spec/setup-admin.md#req-adm-039-overview-mesh-status-cards)) ([REQ-ADM-020](../../sdd/spec/setup-admin.md#req-adm-020-node-status-clarity-and-filtering))

The Overview carries no activity feed: the activity log renders in Settings only. ([REQ-OBS-010](../../sdd/spec/observability.md#req-obs-010-live-throughput-surface))
...
Node and model drawers show split stage ownership as layer ranges (for example `L0-26 → battlestation · Ready`) instead of a bare count when `stageAssignments` are present, inferring `Ready` from the owning node when MeshLLM omits state on a topology row. Runtime binary installation, operator deactivation, and model loading are separate signals: installing `llama-server` or MeshLLM is a runtime-install chip, a deactivated node shows a separate `Deactivated` status while the runtime chip remains version-only when installed or paused when no version is known, and switching/downloading/loading a model is work state; the runtime chip leads with the runtime's name (`llama.cpp b9928`, `meshllm 0.72.2`), never a bare version. ([REQ-OBS-007](../../sdd/spec/observability.md#req-obs-007-mesh-health-surface)) ([REQ-OBS-012](../../sdd/spec/observability.md#req-obs-012-runtime-status-detail-surface)) ([REQ-ADM-033](../../sdd/spec/setup-admin.md#req-adm-033-runtime-binary-version-and-install-visibility))

The Mesh & Models section carries the Meshes management card — the machine groups listed with a right-aligned route chip and machine/model counts, a "+ Mesh" header disclosure revealing the letters-only name input, and per-mesh delete that succeeds only when the mesh is empty — and the nodes table shows each machine's mesh in a Mesh column (no per-model column — model detail lives in the node drawer).

Every models-list row carries a fixed pill vocabulary — provider (`llama.cpp` red / `meshllm` green), serving mode (`singular model` blue / `sharded model` orange), and the purple mesh pill resolving the group name — with the same pill row leading the model Manage drawer above its status and alias fields; mesh participation, stage owners, and the machine group appear once, in the drawer's mesh card and its Technical details.

The node and model Manage drawers each expose a Mesh select that is sent only when the operator changes it (a node move posts through the node-config endpoint; a model move posts through the profile-config endpoint and arrives switched off in the new mesh).

The add-model form sits behind a "+ Model" disclosure at the right end of the Models list header and includes a "Where to find models" sources panel: the `repo:quant` reference-format example plus Unsloth GGUF, mesh-llm layer packages, and prepare-your-own rows, contextual to the selected serving mode.

The model drawer also offers a Duplicate control for any model — it clones the profile into a switched-off same-mesh copy (`<name> (copy)`, derived call name) through the profile-duplicate endpoint. ([REQ-SCH-006](../../sdd/spec/state-scheduling.md#req-sch-006-mesh-registry-and-membership)) ([REQ-ADM-023](../../sdd/spec/setup-admin.md#req-adm-023-per-node-settings)) ([REQ-ADM-021](../../sdd/spec/setup-admin.md#req-adm-021-model-serving-configuration)) ([REQ-ADM-025](../../sdd/spec/setup-admin.md#req-adm-025-add-a-model-console-control)) ([REQ-ADM-037](../../sdd/spec/setup-admin.md#req-adm-037-console-mesh-management)) ([REQ-ADM-038](../../sdd/spec/setup-admin.md#req-adm-038-console-mesh-assignment-surface)) ([REQ-RUN-017](../../sdd/spec/runtime-profiles.md#req-run-017-profile-duplication))
...
Live tokens-per-second are drawn as a rolling-window trace when runtimes report an aggregate throughput signal: each poll appends the aggregate throughput as a bar and the client linearly smooths between successive samples, capping the series at the configured window. Direct llama.cpp machines report live prompt/generation throughput derived from llama-server's cumulative Prometheus token counters — the agent samples them each heartbeat tick and reports the deltas over the tick window, so an idle machine honestly reports zero. Because MeshLLM does not reliably expose per-node live throughput for client/standby roles, the Nodes table and node drawer omit per-node `tok/s` instead of filling the UI with `not reported`. ([REQ-OBS-009](../../sdd/spec/observability.md#req-obs-009-hardware-and-throughput-metrics)) ([REQ-OBS-010](../../sdd/spec/observability.md#req-obs-010-live-throughput-surface))
...
Speed Test results persist per resolved model, and each Overview mesh card shows its own model's latest prompt/generation split, so ingestion and generation speed stay separate per model. All values derive from the status contract — nothing is fabricated between polls. ([REQ-OBS-010](../../sdd/spec/observability.md)) ([REQ-ADM-039](../../sdd/spec/setup-admin.md#req-adm-039-overview-mesh-status-cards)) The Models section lists active profiles before standby ones, preserving source order within each group, so the serving set is visible without scrolling. ([REQ-ADM-018](../../sdd/spec/setup-admin.md#req-adm-018-models-section-ordering))
...
Direct llama.cpp metrics also carry the selected/installed version, verified managed backend family, cache settings, slot state, latest cached-token count, and whether the current multimodal model cannot use llama.cpp's cross-divergence reuse optimization. Ordinary text prefix caching remains available when prompt caching is enabled. Custom or host binaries whose build is not verified report backend `unknown`; bootstrap failures remain `dependency-missing`. ([REQ-OBS-003](../../sdd/spec/observability.md#req-obs-003-node-metrics)) ([REQ-OBS-013](../../sdd/spec/observability.md#req-obs-013-direct-runtime-capability-reporting)) ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap)) ([REQ-NODE-016](../../sdd/spec/node-agent.md#req-node-016-direct-runtime-backend-identity))

Direct vLLM metrics carry runtime kind `vllm`, the effective vLLM version, and the observed host capabilities (`platform`, `cudaAvailable`) — serialized only when actually probed, because the scheduler's capability gate fails closed on absence. Live prompt and generation throughput are counter deltas from vLLM's cumulative Prometheus token counters (tolerating `_total` suffixes and label sets, filtered to the served model), and the in-flight count is the engine's running plus waiting requests. Absent series stay absent rather than zero-filled. ([REQ-OBS-003](../../sdd/spec/observability.md#req-obs-003-node-metrics)) ([REQ-OBS-009](../../sdd/spec/observability.md#req-obs-009-hardware-and-throughput-metrics)) ([REQ-OBS-014](../../sdd/spec/observability.md#req-obs-014-admin-runtime-diagnostics)) ([REQ-NODE-016](../../sdd/spec/node-agent.md#req-node-016-direct-runtime-backend-identity))

Heartbeats also carry `runtimeDetail`, the most recent error-looking line the agent captured from the managed runtime's own stderr (an OOM, a CUDA error, a lane that never came up), plus `nodeState`. ([REQ-OBS-011](../../sdd/spec/observability.md#req-obs-011-runtime-error-surface)) ([REQ-OBS-012](../../sdd/spec/observability.md#req-obs-012-runtime-status-detail-surface))

mesh-llm and llama.cpp share one capture ring, so the chatter gate reads both level spellings: mesh-llm's whole-word JSON level and llama.cpp's bare leading severity letter. A non-error-leveled line is ignored unless it carries a hard error token, and an error marker counts only where it stands at a word start, so neither routine chatter nor a marker buried in a longer word (`oom` inside `making room for prompt cache entry`) can masquerade as the failure cause. ([REQ-OBS-011](../../sdd/spec/observability.md#req-obs-011-runtime-error-surface))

A failed install state derives only from the agent's `dependency-missing` report, so a starting runtime with stderr chatter stays `pending`. For MeshLLM, `nodeState` is the console's raw node state string; during a profile/model restart, the agent sets `nodeState` to the model being loaded so the control plane can distinguish model acquisition from runtime-binary installation. ([REQ-OBS-011](../../sdd/spec/observability.md#req-obs-011-runtime-error-surface)) ([REQ-OBS-012](../../sdd/spec/observability.md#req-obs-012-runtime-status-detail-surface))
...
The node Manage drawer renders current runtime errors, derived work state, mesh role, peers, stage ownership, reachability, and runtime version. The agent clears startup errors on readiness, so a later non-chatter error remains visible even if the runtime still serves; warning/info chatter and stale `model_size_unknown` diagnostics stay hidden. ([REQ-OBS-014](../../sdd/spec/observability.md#req-obs-014-admin-runtime-diagnostics)) ([REQ-OBS-012](../../sdd/spec/observability.md#req-obs-012-runtime-status-detail-surface)) ([REQ-OBS-007](../../sdd/spec/observability.md#req-obs-007-mesh-health-surface))
...
| Mesh health | [observability.md](../../sdd/spec/observability.md) | `packages/router-worker/src/handlers/status.ts::handleAdminStatus`, `packages/router-worker/src/mesh-state.ts::MESH_STATE_ANCHORS` <!-- @impl: packages/router-worker/src/handlers/status.ts::handleAdminStatus --> <!-- @impl: packages/router-worker/src/mesh-state.ts::MESH_STATE_ANCHORS --> |
...
| Mesh registry and membership | [state-scheduling.md](../../sdd/spec/state-scheduling.md) | `packages/router-worker/src/meshes.ts::MESHES_ANCHORS`, `packages/router-worker/src/handlers/meshes.ts::meshListCore` <!-- @impl: packages/router-worker/src/meshes.ts::MESHES_ANCHORS --> <!-- @impl: packages/router-worker/src/handlers/meshes.ts::meshListCore --> |
| Current runtime manager | [observability.md](../../sdd/spec/observability.md) | `packages/node-agent/cmd/inference-mesh-agent/service_loop.go::setManager`, `packages/node-agent/cmd/inference-mesh-agent/service_loop.go::dashboardStatus`, `packages/node-agent/cmd/inference-mesh-agent/service_loop.go::currentRuntimeController` <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/service_loop.go::setManager --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/service_loop.go::dashboardStatus --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/service_loop.go::currentRuntimeController --> |
