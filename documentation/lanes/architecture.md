...
4. Worker maps the requested stable public model id to that mesh's single active model profile via `getProfileByPublicModel`: the default mesh's active model answers `codeflare-mesh` and every other mesh's active model answers `codeflare-mesh-<mesh>`, each profile carrying its mesh's stable alias while per-mesh single-active leaves exactly one owner. ([REQ-RUN-001](../../sdd/spec/runtime-profiles.md#req-run-001-stable-public-model)) ([REQ-SCH-006](../../sdd/spec/state-scheduling.md#req-sch-006-mesh-registry-and-membership))
