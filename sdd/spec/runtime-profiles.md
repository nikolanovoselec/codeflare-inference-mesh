...
5. A direct profile's Manage drawer exposes parallel slots (blank = Auto), a unified-KV toggle defaulting to on, GPU layers, KV cache, batch, micro-batch, flash attention, generation cap, prompt cache/reuse, and reasoning settings, and hides MeshLLM-only controls. <!-- @impl: packages/router-worker/src/client/drawers.ts::openModelDrawer --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-RUN-013 loads and saves direct llama.cpp runtime tunables from the model drawer) --> <!-- @test: packages/router-worker/src/router-observability.test.ts (REQ-OBS-012 REQ-RUN-013 keeps direct llama.cpp UI controls backed by admin API payloads) -->

6. Saving direct profile settings posts through `POST /admin/profiles/config` with a `llamacpp` settings block whose parallel and context-window fields accept their Auto sentinels or documented fixed ranges. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::ADMIN_UI_CLIENT_SCRIPT --> <!-- @impl: packages/router-worker/src/profile-config.ts::resolveLlamaCppSettings --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-RUN-013 loads and saves direct llama.cpp runtime tunables from the model drawer) --> <!-- @test: packages/router-worker/src/router-admin.test.ts (REQ-ADM-021 configures direct llama.cpp settings through the admin profile config path) -->
...
1. The model drawer exposes a per-profile projector toggle and saves its selected boolean state. <!-- @impl: packages/router-worker/src/client/drawers.ts::openModelDrawer --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-RUN-013 loads and saves direct llama.cpp runtime tunables from the model drawer) -->
