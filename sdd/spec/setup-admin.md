...
3. An admin or automation caller's run updates the stored summary; a read-only console viewer receives the same measurement and the stored summary is left unchanged. <!-- @impl: packages/router-worker/src/speed-test.ts::runSpeedTest --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-034 REQ-SEC-010 measures a read-only viewer speed test without overwriting the stored summary) -->
