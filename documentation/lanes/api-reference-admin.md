...
Direct llama.cpp profiles accept a `llamacpp` block with `parallel` (`-1` = Auto, else `>= 1`), `kvUnified` (boolean; `null` clears back to on; `false` together with Auto parallel is rejected), `gpuLayers` (`0` or a positive integer, or `"auto"` / `"all"`; `null`/`""` clears), `cachePrompt` (boolean), `cacheReuse` (`>= 0`), `cacheTypeK` / `cacheTypeV` (`f32` \| `f16` \| `bf16` \| `q8_0` \| `q4_0` \| `q4_1` \| `iq4_nl` \| `q5_0` \| `q5_1`), `batch`, `ubatch`, `maxOutputTokens` (positive integers; `null`/`0` clears), `flashAttn` (boolean; `null` clears), `mmproj` (boolean; `null` clears back to llama.cpp's default auto-load, `false` renders `--no-mmproj` to opt a text workload out of the multimodal projector), `reasoning` (`{ enabled?, format?, budget? }`, `null` clears), optional `bindPort` (reserved bind ports are rejected), and the launch source `hfRepo`, `quant`, and optional `hfFile` (a per-file override inside the repo). The node agent reads only `hfRepo`/`quant`/`hfFile` — never `modelRef` — so the reference is the source of truth: a changed `modelRef` re-derives `hfRepo`/`quant` (dropping a stale `hfFile`), a save whose source no longer reconstructs from the reference is refused with `model_source_mismatch`, and a `quant` tag ending in `.` or containing whitespace is refused with `invalid_quant_tag`.
