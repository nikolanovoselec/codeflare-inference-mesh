...
6. Re-running sync patches drift on the existing provider — matched by a slug derived from the provider name alone, never the Worker origin — before route deployment, so a changed Worker URL cannot mint a duplicate provider that lacks the BYOK key. <!-- @impl: packages/router-worker/src/cloudflare-api.ts::syncCustomProvider --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 patches an existing Gateway provider when Worker URL drifts) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 keeps the provider slug stable across worker origins so a re-sync reconciles instead of duplicating) -->
