...
11. Admin can add a model beyond seeded defaults from the dashboard or `POST /admin/profiles/add`. The profile joins inactive and serves only after activation.
    - MeshLLM models can be single-machine or split; llama.cpp models are direct single-machine profiles for cache-local coding sessions. ([REQ-RUN-011](../../sdd/spec/runtime-profiles.md#req-run-011-custom-model-onboarding)) ([REQ-ADM-025](../../sdd/spec/setup-admin.md#req-adm-025-add-a-model-console-control))
