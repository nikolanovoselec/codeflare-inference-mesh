...
11. Admin can add a model beyond the seeded defaults via the dashboard add-model form or `POST /admin/profiles/add`, supplying a model reference, serving mode, and runtime. MeshLLM models can be single-machine or split; llama.cpp models are direct single-machine profiles for cache-local coding sessions. The profile joins the catalog inactive and only serves once activated. ([REQ-RUN-011](../../sdd/spec/runtime-profiles.md#req-run-011-custom-model-onboarding)) ([REQ-ADM-025](../../sdd/spec/setup-admin.md#req-adm-025-add-a-model-console-control))
