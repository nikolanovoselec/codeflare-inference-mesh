...
7. When a custom-provider call omits `body.user` but includes metadata visible to the router (`cf-aig-metadata` if forwarded, or a JSON `metadata` body object) with `user` and optional `session`, the router derives `user:<metadata.user>|session:<metadata.session-or-user>` before direct scheduling. When AI Gateway REST dynamic-route metadata is visible only in Gateway logs and not forwarded, the router falls back to `user:ai-gateway|session:provider-default` for provider-authenticated calls. <!-- @impl: packages/router-worker/src/router.ts::directSessionBody --> <!-- @impl: packages/router-worker/src/router.ts::gatewayMetadataDirectSession --> <!-- @impl: packages/router-worker/src/router.ts::providerDefaultDirectSession --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-004 derives direct llama.cpp session affinity from AI Gateway metadata) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-004 uses a provider-scoped fallback session when Gateway metadata is not forwarded) -->
