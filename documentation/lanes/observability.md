...
Heartbeats also carry `runtimeDetail` — the most recent error-looking line the agent captured from mesh-llm's own stderr (an OOM, a CUDA error, a lane that never came up) — and `nodeState`, the console's raw node state string. Together they make the reason a runtime is wedged legible from the control plane instead of only the host journal; the agent tees mesh-llm's stderr through a bounded ring that keeps the latest error line, and a zero value never overwrites a present one on merge. The console node Manage drawer renders these as a read-only diagnostics panel: the runtime error as a highlighted row when present, alongside node state, mesh role, peer and stage counts, API/console reachability, and the mesh-llm version. ([REQ-OBS-011](../../sdd/spec/observability.md))
