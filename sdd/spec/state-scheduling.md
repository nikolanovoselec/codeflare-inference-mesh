...
3. Claim and heartbeat responses carry only the profiles of the node's own mesh, and a newly claimed node joins the default mesh. <!-- @impl: packages/router-worker/src/router.ts::meshProfilesFor --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-006 a newly claimed node joins the default mesh and receives only default-mesh profiles) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-006 heartbeat sends only the node's mesh profiles and no foreign mesh bootstrap) -->

4. A heartbeat from a node still self-reporting a foreign mesh's profile ids receives no mesh bootstrap for that mesh and cannot re-add its invite token to that mesh's state. <!-- @impl: packages/router-worker/src/router.ts::selectedMeshProfile --> <!-- @impl: packages/router-worker/src/mesh-state.ts::selectedMeshProfile --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-006 heartbeat sends only the node's mesh profiles and no foreign mesh bootstrap) -->
...
**Dependencies:** [REQ-SCH-001](#req-sch-001-durable-router-state), [REQ-SCH-003](#req-sch-003-node-eligibility-and-scheduler-miss-responses), [REQ-RUN-016](runtime-profiles.md#req-run-016-per-mesh-model-assignment)
