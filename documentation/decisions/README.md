...
**Update (2026-08-21):** the deferred cleanup migration has landed. The `reservations` and `sessions` tables no longer exist, so restoring the reservation subsystem would now also require a schema migration. ([REQ-SCH-001](../../sdd/spec/state-scheduling.md#req-sch-001-durable-router-state))
