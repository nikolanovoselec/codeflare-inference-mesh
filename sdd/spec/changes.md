...
- Updated [REQ-SCH-003](state-scheduling.md#req-sch-003-node-eligibility-and-scheduler-miss-responses) so scheduler miss responses identify no-profile as `404` and no-node as `429` with request IDs instead of a retry hint.
