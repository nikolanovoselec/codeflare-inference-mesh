...
| Speed test | Measures one streamed request through the real forward path under a bounded deadline; an admin or automation run stores the result per resolved profile, a read-only viewer's run does not. | `packages/router-worker/src/speed-test.ts` | [REQ-ADM-034](../../sdd/spec/setup-admin.md#req-adm-034-direct-router-speed-test) |
