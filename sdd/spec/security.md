...
5. Admin can revoke a node token and remove the node from eligible scheduling. <!-- @impl: packages/router-worker/src/router.ts::handleNodeRevoke --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SEC-002 lets an admin revoke a node and audit the action) -->
