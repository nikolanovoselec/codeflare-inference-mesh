...
3. When MeshLLM omits GPU capacity or used VRAM, host GPU telemetry fills the missing values without overwriting MeshLLM's trusted total; on Apple Silicon, where `system_profiler` reports no VRAM line, the darwin fallback derives the GPU budget from unified memory (`hw.memsize`) at Metal's ~75% working-set convention and reads live consumption from the IORegistry accelerator in-use counter (degrading to unknown, never a fabricated value), so a Mac shows used/total VRAM under both runtimes. <!-- @impl: packages/node-agent/internal/agent/gpu.go::GPUFallbackMetrics --> <!-- @impl: packages/node-agent/internal/agent/gpu.go::appleUnifiedMemoryBudgetMiB --> <!-- @impl: packages/node-agent/internal/agent/gpu.go::appleGPUInUseMiB --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/main.go::collect --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/main.go::execCommandRunner --> <!-- @test: packages/node-agent/internal/agent/agent_test.go (TestREQOBS009GPUFallbackPerOSAndMerge) --> <!-- @test: packages/node-agent/internal/agent/gpu_test.go (TestREQOBS009AppleSiliconUnifiedMemoryBudget) --> <!-- @test: packages/node-agent/internal/agent/gpu_test.go (TestREQOBS009AppleSiliconInUseUnavailableStaysUnknown) --> <!-- @test: packages/node-agent/internal/agent/agent_test.go (TestREQOBS009BestEffortHardwareMetrics) --> <!-- @test: packages/node-agent/cmd/inference-mesh-agent/main_test.go (TestREQOBS009CollectFillsMeshLLMUsedVRAMFromHostTelemetry) -->
...
6. Under direct llama.cpp, live prompt and generation throughput derive from llama-server's cumulative Prometheus token counters sampled once per heartbeat tick: the rates are the counter deltas over the tick window, an idle window reports zero, and a failed poll or a counter that goes backwards (llama-server restarted) zeroes the rates and reseeds instead of fabricating a delta. <!-- @impl: packages/node-agent/internal/agent/llamacpp_manager.go::PollThroughput --> <!-- @impl: packages/node-agent/internal/agent/llamacpp_manager.go::parseLlamaCounters --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/main.go::collect --> <!-- @test: packages/node-agent/internal/agent/llamacpp_manager_test.go (TestREQOBS009LlamaCppLiveThroughputFromCounterDeltas) --> <!-- @test: packages/node-agent/internal/agent/llamacpp_manager_test.go (TestREQOBS009LlamaCppThroughputResetsOnRestartAndFailure) -->
