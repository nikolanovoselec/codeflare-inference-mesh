...
9. Node projections carry `displayStatus` — the router-derived operator status vocabulary (`Serving`/`Preparing`/`Disconnected`/`Offline`/`Error` plus lifecycle labels) — so automation reads the same status the console shows. <!-- @impl: packages/router-worker/src/router.ts::nodeDisplayStatus --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-020 REQ-API-004 node projections carry the derived operator status vocabulary) -->
