...
4. Nodes report ready profiles with profile ID, version, loaded model, and runtime state. <!-- @impl: packages/router-worker/src/handlers/status.ts::handleAdminStatus --> <!-- @test: packages/router-worker/src/router-admin.test.ts (REQ-ADM-002 REQ-OBS-002 returns redacted machine-readable admin status and REQ-RUN-004 reports profile readiness in admin status) -->
...
6. Saving direct profile settings posts through `POST /admin/profiles/config` with a `llamacpp` settings block whose parallel and context-window fields accept their Auto sentinels or documented fixed ranges. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::ADMIN_UI_CLIENT_SCRIPT --> <!-- @impl: packages/router-worker/src/profile-config.ts::resolveLlamaCppSettings --> <!-- @test: packages/router-worker/src/admin-ui-models.test.ts (REQ-RUN-013 loads and saves direct llama.cpp runtime tunables from the model drawer) --> <!-- @test: packages/router-worker/src/router-admin.test.ts (REQ-ADM-021 configures direct llama.cpp settings through the admin profile config path) -->

7. Disabling unified KV together with Auto parallel is rejected with status 400. <!-- @impl: packages/router-worker/src/profile-config.ts::resolveLlamaCppSettings --> <!-- @test: packages/router-worker/src/router-admin.test.ts (REQ-ADM-021 configures direct llama.cpp settings through the admin profile config path) -->
