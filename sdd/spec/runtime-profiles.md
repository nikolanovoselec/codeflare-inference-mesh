...
3. The agent stops the runtime with SIGTERM first and escalates to kill only after a grace period; the drain gate counts a direct vLLM runtime's in-flight work as its running plus waiting engine requests. <!-- @impl: packages/node-agent/internal/agent/meshllm_manager.go::MeshLLMManager --> <!-- @impl: packages/node-agent/internal/agent/vllm_manager.go::VllmManager --> <!-- @test: packages/node-agent/internal/agent/meshllm_manager_test.go (TestREQRUN010StopSendsSIGTERMBeforeKill) --> <!-- @test: packages/node-agent/internal/agent/vllm_manager_test.go (TestREQRUN010VllmInflightCountsRunningAndWaiting) -->
