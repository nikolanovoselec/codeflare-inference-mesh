...
| `AI_GATEWAY_ROUTE_NAME` | `codeflare-mesh` | no | Declared in Wrangler but not consumed; the default mesh's dynamic route name is pinned to the stable public model `codeflare-mesh` in `packages/router-worker/src/router.ts::gatewaySettings`, and non-default meshes' route names derive from the mesh registry (`codeflare-mesh-<id>`). | [REQ-GWY-003](../../sdd/spec/gateway.md) |
