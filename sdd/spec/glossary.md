...
| Runtime | Supervised inference process on the node — `mesh-llm`, `llama-server`, or `vllm serve` — that exposes the local OpenAI-compatible inference API. |
...
| vLLM | Python inference engine (`vllm serve`) for Hugging Face safetensors models on NVIDIA CUDA; runs as a direct single-node runtime installed into a pinned per-version virtual environment. See [REQ-RUN-021](runtime-profiles.md#req-run-021-direct-vllm-custom-profiles). |
...
| Reservation | Retired scheduler record that once pinned one request to one node; the stateless forwarder no longer creates reservations and the `reservations` D1 table has been dropped. See [REQ-SCH-002](state-scheduling.md#req-sch-002-stateless-entry-node-forwarding). |
| Backend Family | The llama.cpp build variant (for example cpu, cuda, metal) verified from a managed release archive; a custom or host-installed binary the installer did not verify reports backend `unknown`. See [REQ-NODE-016](node-agent.md#req-node-016-direct-runtime-backend-identity). |
| Cross-Divergence Reuse | Multimodal prompt-cache optimization that reuses cached tokens across prompts whose image content differs; distinct from ordinary text prefix caching, and unavailable for some direct models. See [REQ-OBS-013](observability.md#req-obs-013-direct-runtime-capability-reporting). |
