...
8. The model and version endpoints refuse a request that carries no valid automation key. <!-- @impl: packages/router-worker/src/router.ts::handleApiModelList --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-API-005 refuses model and version endpoints without an automation key) -->
