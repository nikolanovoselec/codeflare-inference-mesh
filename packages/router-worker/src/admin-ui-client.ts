...
export const ADMIN_UI_CLIENT_SCRIPT: string = `(() => {
  ...
  function openModelDrawer(profileId) {
    ...
    if (isDirect) {
      ...
      bodyEl.appendChild(meshTunableNumberRow({ id: 'model-edit-llama-batch', label: 'Prefill batch', value: llamacpp.batch, placeholder: '8192', hint: 'llama.cpp --batch-size. Higher values speed long-prompt ingestion but use more memory.' }));
      bodyEl.appendChild(meshTunableNumberRow({ id: 'model-edit-llama-ubatch', label: 'Micro-batch', value: llamacpp.ubatch, placeholder: '2048', hint: 'llama.cpp --ubatch-size. Raise carefully when the GPU has memory headroom; lower it if the runtime fails to load.' }));
      ...
      bodyEl.appendChild(meshTunableNumberRow({ id: 'model-edit-llama-maxout', label: 'Generation cap (-n / --predict)', value: llamacpp.maxOutputTokens, placeholder: '8192', hint: 'llama.cpp server-side default/max tokens to predict. Requests may still pass max_tokens; this value maps to -n N / --predict N and should stay above the reasoning budget.' }));
