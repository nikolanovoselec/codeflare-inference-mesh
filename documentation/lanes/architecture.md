...
| Request dispatcher | Rate limiting, default-profile seeding, the custom-domain host gate, credential enforcement, and error mapping; then one dispatch over the route table. Owns no route's logic. | `packages/router-worker/src/router.ts` | [REQ-RTR-001](../../sdd/spec/router-worker.md) |
