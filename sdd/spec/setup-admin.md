...
2. `POST /admin/nodes/{id}/config` persists a non-blank display name in the D1 node JSON, sets or clears a node's VRAM override, rejects a blank name or negative VRAM value, and returns `404` for an unknown or revoked node. <!-- @impl: packages/router-worker/src/handlers/nodes.ts::handleNodeConfig --> <!-- @test: packages/router-worker/src/router-admin-nodes.test.ts (REQ-ADM-023 persists node name and VRAM override across heartbeat) --> <!-- @test: packages/router-worker/src/router-api.test.ts (REQ-ADM-023 refuses reconfigure and admin config for a revoked node) -->
...
4. `POST /api/v1/nodes/{id}/reconfigure` persists the same display name and VRAM override for an automation caller, returns the node projection including them, and returns `404` for an unknown or revoked node. <!-- @impl: packages/router-worker/src/handlers/nodes.ts::handleApiNodeReconfigure --> <!-- @test: packages/router-worker/src/router-api.test.ts (REQ-ADM-023 reconfigures node name and VRAM override through the automation API) --> <!-- @test: packages/router-worker/src/router-api.test.ts (REQ-ADM-023 refuses reconfigure and admin config for a revoked node) -->
...
3. `POST /admin/profiles/delete` under admin authentication removes the named switched-off model. <!-- @impl: packages/router-worker/src/handlers/models.ts::handleProfileDelete --> <!-- @test: packages/router-worker/src/router-api.test.ts (REQ-ADM-026 deletes a custom model from the console) -->

4. The console delete refuses only the active model, with status 409. <!-- @impl: packages/router-worker/src/handlers/models.ts::classifyModelDeletion --> <!-- @test: packages/router-worker/src/router-api.test.ts (REQ-ADM-026 refuses console deletion only while the model is active) -->

5. The console delete refuses a request that carries no admin credential. <!-- @impl: packages/router-worker/src/handlers/models.ts::handleProfileDelete --> <!-- @test: packages/router-worker/src/router-api.test.ts (REQ-ADM-026 refuses console model deletion without an admin credential) -->
