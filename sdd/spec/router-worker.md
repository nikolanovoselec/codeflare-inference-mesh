...
7. Every route that declares a credential class refuses an uncredentialed caller, and only routes declared open answer one. <!-- @impl: packages/router-worker/src/auth-gates.ts::AUTH_GATES_ANCHORS --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RTR-001 REQ-SEC-001 refuses every gated route without a credential and keeps each gate in its route family) -->
