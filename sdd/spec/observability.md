...
1. The Overview hero stats strip reports mesh-ready node counts, total mesh VRAM, and the latest Speed Test prompt/generation throughput from admin status when a summary exists; split MeshLLM participants that have a ready model/API path count as mesh-ready capacity even when their raw MeshLLM role is client-like. <!-- @impl: packages/router-worker/src/admin-ui-views.ts::dashboardView --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::renderStatus --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-010 computes the stats strip aggregates from admin status) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-010 counts split API-client participants as mesh-ready capacity) -->
