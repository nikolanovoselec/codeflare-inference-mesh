...
| Set runtime secrets | The fail-closed mesh-state and session-affinity values come from the [private deployment matrix](https://github.com/nikolanovoselec/codeflare-inference-mesh-private); deployment stops when either is absent. | [REQ-SEC-006](../../sdd/spec/security.md), [REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity) |
...
For defence in depth, add a Cloudflare Zero Trust Gateway network policy, scoped to the enrolled mesh devices, that blocks any iroh/QUIC egress to a destination outside `100.96.0.0/12`, so a compromised node cannot push inference data out through a public iroh relay. This is the operator-provisioned network-layer backstop behind the `--disable-iroh-relays` launch flag; see [security.md](security.md#mesh-egress-posture). ([REQ-SEC-004](../../sdd/spec/security.md#req-sec-004-runtime-api-exposure))

No separate MeshLLM install is required: the installed agent bootstraps the operator-selected MeshLLM release itself. MeshLLM defaults to `mesh-llm` v0.72.2 until changed in Settings/API; the agent downloads the per-platform release archive, verifies the embedded default or selected checksum sidecar, extracts `mesh-llm`, and installs it by atomic rename. MeshLLM launch config, version selection, and content-change restarts come from router-delivered runtime settings. ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap), [REQ-RUN-014](../../sdd/spec/runtime-profiles.md#req-run-014-meshllm-runtime-configuration-and-versioning))

The MeshLLM build flavor is auto-detected: CUDA runtime major on `nvidia-smi` Linux hosts (`cuda-13` or `cuda-12`), `metal` on darwin/arm64, and `cpu` when no GPU is present. Both CUDA selections map to upstream's `--llama-flavor cuda`; a mismatch fails at `libcudart.so.<major>` load, so install verification surfaces before scheduling. ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap))

Direct llama.cpp profiles stay single-node/cache-local and launch `llama-server`. The agent first looks for a host CUDA-enabled `llama-server` on PATH and common install paths, then falls back to the best matching upstream release asset, verifies its digest, installs the binary plus runtime shared libraries under the agent data directory, and launches with that directory on the library path. If the service account cannot see the known-good host install, set `llamaCppBinaryPath`; the agent then skips the managed asset and runs that host binary. ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap), [REQ-RUN-013](../../sdd/spec/runtime-profiles.md#req-run-013-direct-llamacpp-custom-profiles))

A managed runtime binary already on PATH is used only when its version matches the selected runtime version. Direct llama.cpp also checks common host install paths before downloading. A configured `llamaCppBinaryPath` is an explicit node-local override for direct llama.cpp and is not replaced by the managed downloader. Install failure reports `dependency-missing`, keeping the node up but never eligible for scheduling, and the Admin UI/API show desired version, installed version when known, install state, and error. ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap)) ([REQ-SCH-003](../../sdd/spec/state-scheduling.md)) The supervised MeshLLM process runs with `MESH_LLM_NO_SELF_UPDATE=1`, so runtime versions move only through the router-selected runtime-version setting. ([REQ-RUN-010](../../sdd/spec/runtime-profiles.md#req-run-010-meshllm-process-lifecycle))
...
1. Configure the required GitHub Actions secrets listed in the [private deployment matrix](https://github.com/nikolanovoselec/codeflare-inference-mesh-private), deploy integration first, and confirm missing required values stop the workflow. ([REQ-SEC-006](../../sdd/spec/security.md), [REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity))
...
5. Confirm an end-to-end chat completion through the AI Gateway dynamic route. For a direct llama.cpp profile, include OpenAI `body.user` as `user:<id>|session:<id>` and verify repeated prompts report increasing `usage.prompt_tokens_details.cached_tokens`. ([REQ-GWY-003](../../sdd/spec/gateway.md), [REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity))
...
5. **Rotation test.** Rotate the mesh token from the Admin UI. Expect `rotation` to increment, mesh state to reset, token count to recover, and the mesh to reform within two minutes.
   - Run this under idle or short-stream load.
   - Model readiness restoration additionally waits on model reload. ([CON-SEC-003](../../sdd/spec/constraints.md#con-sec-003-mesh-secret-custody-and-rotation)) ([REQ-SEC-006](../../sdd/spec/security.md))
...
| Session affinity key deploy | [state-scheduling.md](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity) | `.github/workflows/deploy.yml::SESSION_AFFINITY_KEY` <!-- @impl: .github/workflows/deploy.yml::SESSION_AFFINITY_KEY --> |
