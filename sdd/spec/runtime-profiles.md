...
4. Profile definitions include a display name distinct from the wiring id, public aliases, upstream model name, source mode `meshllm-ref`, an optional context limit (`0` meaning Auto so MeshLLM sizes it), runtime `meshllm`, MeshLLM settings (model ref, split flag, mandatory mesh bind port, optional max VRAM, and the optional runtime tunables: parallel lanes, KV cache type for keys and values, prefill batch, micro-batch, flash attention, max output tokens, and reasoning), profile version, rollout percent, and active flag. Each tunable is optional; an omitted value means Auto and is not rendered into the node config. <!-- @impl: packages/router-worker/src/profiles.ts::PROFILE_ANCHORS --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RUN-002 exposes profile source modes and meshllm contract values) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RUN-002 persists per-model runtime tunables and clears them back to Auto) -->
...
6. When a profile sets any supported MeshLLM configuration value, the agent renders a per-profile config file passed as `--config <data dir>/meshllm-<profileId>.toml`, emitting each set value under its MeshLLM subtable: `[models.model_fit]` for the context limit, prefill batch, micro-batch, KV cache types, and flash attention; `[models.throughput]` for parallel lanes; and `[models.request_defaults]` for max output tokens and the reasoning enable/format/budget. A value left unset (`0`/empty/absent, including a non-positive context limit) is omitted so MeshLLM auto-plans it; when a profile sets nothing, no config file is written and the values are client-facing metadata only. <!-- @impl: packages/node-agent/internal/agent/meshllm_render.go::MeshLLMConfigTOML --> <!-- @test: packages/node-agent/internal/agent/meshllm_render_test.go (TestREQRUN003ContextLimitConfigRendering) -->
