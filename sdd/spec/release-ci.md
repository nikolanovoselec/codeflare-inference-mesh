...
5. Security checks include lockfile npm audit, Go vulnerability checks, dependency review where the event supports it, pinned workflow actions, and least-privilege workflow token permissions. <!-- @impl: .github/workflows/ci.yml::REL001PullRequestChecks --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-001 runs PR, main-push, manual, router, agent, packaging, security, and aggregate test checks) -->
6. Dependency update pull requests cover npm workspaces, the node-agent Go module, and GitHub Actions. <!-- @impl: .github/dependabot.yml::updates --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-001 opens dependency update pull requests for supported package ecosystems) -->
7. A quality check lints the repository and reports unused files, exports, and dependencies; the aggregate gate asserts its result, so a quality failure blocks the run. <!-- @impl: .github/workflows/ci.yml::REL001PullRequestChecks --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-001 runs PR, main-push, manual, router, agent, packaging, security, and aggregate test checks) -->
...
1. Deploy repeats critical router and agent checks after lockfile dependency install and before changing Cloudflare state. <!-- @impl: .github/workflows/deploy.yml::REL002ManualIntegrationDeploy --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-002 REQ-REL-005 auto-deploys production only after green main gates and allows manual integration from any branch) -->
...
**Dependencies:** [REQ-REL-002](#req-rel-002-deploy-workflow-gating), [REQ-SCH-001](state-scheduling.md#req-sch-001-durable-router-state)
...
1. Deploy builds agent archives for linux/amd64, linux/arm64, windows/amd64, and darwin/arm64 with CGO disabled so the linux binaries link statically and run across glibc distributions, and publishes no darwin/amd64 archive. <!-- @impl: .github/workflows/deploy.yml::REL003ReleaseArtifacts --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-003 builds cross-platform release assets, manifest, optional signature, and GitHub Release) -->
