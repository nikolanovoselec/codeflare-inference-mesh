...
**Cause:** A split profile's stages are incomplete — not every serving node needed for the layer split is online — the model download is still in progress, or MeshLLM's split planner rejected the available budgets. In the capacity case the peers can be connected and the API/console can be up while `/v1/models` stays empty; the key signal is a split-readiness blocker such as `split_capacity_shortfall` with participant VRAM and shortfall bytes.

**Fix:** Compare `stageAssignments` / `stageCount` and `splitReadiness` in node metrics or `/api/v1/status?detail=full` against the online serving nodes for the profile, and check the node drawer's Mesh VRAM budget row. If the profile budget is higher than the launched `--max-vram`, clear or raise that node's VRAM override and wait for the updated agent to restart from the current profile config; old agents reused stale launch inputs on Force Reload. If the node reports `mesh-llm readiness deadline exceeded`, the agent treats the failed runtime as restart-needed on the next heartbeat and should relaunch it automatically. Use Force Reload from the node's Manage drawer or `POST /api/v1/nodes/{id}/reload` only if repeated heartbeats do not self-heal the stalled runtime. ([REQ-RUN-007](../../sdd/spec/runtime-profiles.md#req-run-007-split-serving-via-layer-packages)) ([REQ-RUN-005](../../sdd/spec/runtime-profiles.md#req-run-005-runtime-readiness-and-status-reporting)) ([REQ-ADM-032](../../sdd/spec/setup-admin.md#req-adm-032-node-force-reload))
...
**Cause:** Several distinct issues collapse to this generic Gateway error. If Gateway logs show no provider step or `provider=unknown`, the caller token likely lacks `AI Gateway: Run`. If logs show `provider=custom-codeflare-inference-mesh` and `model=dynamic/codeflare-mesh`, the dynamic-route model name reached the custom provider; the Worker must normalize that prefix to `codeflare-mesh` before profile selection, otherwise MeshLLM routing can fail before it reaches a node. For direct llama.cpp profiles, Gateway's REST dynamic-route path often does not forward user metadata to the custom provider, so the Worker must also include the provider-scoped fallback session for no-forwarded-metadata calls. ([REQ-GWY-003](../../sdd/spec/gateway.md#req-gwy-003-dynamic-route-automation), [REQ-SEC-012](../../sdd/spec/security.md), [REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity))

**Fix:** First confirm the client's Cloudflare token includes `AI Gateway: Run` and is used with `cf-aig-gateway-id`. Then ensure the Worker version includes both dynamic-model normalization (`dynamic/codeflare-mesh` → `codeflare-mesh`) and the direct llama.cpp provider-scoped fallback. If the client can send identity fields, prefer OpenAI `user: "user:<id>|session:<id>"` or body `metadata: { "user": "<id>", "session": "<optional-session>" }`; otherwise the fallback keeps direct routes usable with shared provider-level affinity. Confirm with a chat completion for `dynamic/codeflare-mesh` and inspect Gateway logs for `status_code=200`.
