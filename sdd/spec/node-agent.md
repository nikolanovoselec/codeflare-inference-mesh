...
8. The heartbeat's in-flight count measures requests against the runtime that is live now, so a runtime (re)start — which happens only after the previous runtime has drained — resets the counter to zero instead of letting a request that died with the old runtime ride on as a stuck in-flight forever. <!-- @impl: packages/node-agent/internal/agent/proxy.go::Reset --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/main.go::setManager --> <!-- @test: packages/node-agent/cmd/inference-mesh-agent/main_test.go (TestREQNODE002InFlightResetsOnRuntimeSwap) -->
...
5. The managed llama.cpp install is named after the backend the release archive actually installs — Vulkan on a Linux NVIDIA box, where upstream publishes no CUDA asset — and heartbeat metrics carry that resolved backend next to `llamacppVersion` so an operator on an NVIDIA machine can see they are not on CUDA; a verified install under the former requested-backend name migrates instead of re-downloading. <!-- @impl: packages/node-agent/internal/agent/llamacpp_install.go::ResolvedLlamaCppBackend --> <!-- @impl: packages/node-agent/internal/agent/llamacpp_install.go::llamaCppManagedTarget --> <!-- @impl: packages/node-agent/internal/agent/llamacpp_install.go::EnsureLlamaCpp --> <!-- @impl: packages/node-agent/internal/agent/metrics.go::NodeMetrics --> <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/main.go::runtimeMetrics --> <!-- @test: packages/node-agent/internal/agent/llamacpp_install_test.go (TestREQNODE013LlamaCppResolvedBackendNamesTheArchiveItInstalls) --> <!-- @test: packages/node-agent/internal/agent/llamacpp_install_test.go (TestREQNODE013LlamaCppMigratesRequestedBackendInstallToResolvedName) --> <!-- @test: packages/node-agent/cmd/inference-mesh-agent/main_test.go (TestREQNODE013LlamaCppMetricsCarryResolvedBackend) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-011 direct node drawer does not turn unreported heartbeat fields into failures) -->

6. The direct llama.cpp runtime's Hugging Face model cache is pinned under the agent's data directory (`HF_HOME=<dataDir>/.cache/huggingface`) in the launched process environment, so direct models never accumulate in the agent user's home outside dataDir where disk accounting and cleanup cannot see them; an inherited `HF_HOME` is replaced, not path-list-prepended. <!-- @impl: packages/node-agent/internal/agent/llamacpp_manager.go::llamaCppRuntimeEnvFor --> <!-- @impl: packages/node-agent/internal/agent/llamacpp_manager.go::upsertSingleEnv --> <!-- @test: packages/node-agent/internal/agent/llamacpp_manager_test.go (TestREQNODE013LlamaCppLaunchEnvPinsHuggingFaceCacheToDataDir) --> <!-- @test: packages/node-agent/internal/agent/llamacpp_manager_test.go (TestREQNODE013LlamaCppLaunchEnvLeavesHuggingFaceCacheUnsetWithoutDataDir) -->
