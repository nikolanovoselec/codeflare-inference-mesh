...
2. The Admin can enter a custom-domain hostname without using zone IDs in the normal setup path. <!-- @impl: packages/router-worker/src/admin-ui.ts::adminUiHtml --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-005 validates and stores optional custom-domain hostnames before accepting them) -->
...
5. The setup UI records the selected custom domain in D1 and accepts a zone resource identifier only from advanced API callers. <!-- @impl: packages/router-worker/src/router.ts::handleCustomDomain --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-005 validates and stores optional custom-domain hostnames before accepting them) -->
