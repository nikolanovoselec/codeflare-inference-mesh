...
No separate MeshLLM install is required: the installed agent bootstraps the operator-selected MeshLLM release itself. MeshLLM defaults to `mesh-llm` v0.72.2 until changed in Settings/API; the agent downloads the per-platform release archive, verifies the default against the embedded SHA-256 map (or the selected release's checksum sidecar), extracts `mesh-llm`, and installs it by atomic rename. The build flavor is auto-detected — on `nvidia-smi` hosts by the installed CUDA runtime major (`cuda-13` on Linux carrying CUDA 13 runtime libraries, `cuda-12` otherwise), `metal` on darwin/arm64, `cpu` when no GPU is present — because the CUDA build dlopens `libcudart.so.<major>` at startup and a mismatched build fails at library load; both `cuda-12` and `cuda-13` map to upstream's `--llama-flavor cuda`. Direct llama.cpp profiles stay single-node/cache-local and launch `llama-server`: the agent first looks for a host-installed CUDA-enabled `llama-server` on PATH and in common install locations, then falls back to the best matching upstream release asset for the detected backend, verifies the GitHub asset digest, installs the managed binary plus runtime shared libraries and chained shared-library link entries under the agent data directory, and launches with that directory on the library path. If the service account still cannot see the known-good host install, set `llamaCppBinaryPath` in the node config to that binary path; the agent then skips the managed asset and runs that host-installed binary. ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap), [REQ-RUN-011](../../sdd/spec/runtime-profiles.md#req-run-011-custom-model-onboarding))
