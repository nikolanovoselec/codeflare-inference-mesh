...
5. The default mesh's active model answers `codeflare-mesh`; every other mesh's active model answers `codeflare-mesh-<mesh>` — each profile carries its mesh's stable alias, and per-mesh single-active leaves exactly one owner per alias. ([REQ-RUN-016](../../sdd/spec/runtime-profiles.md#req-run-016-per-mesh-model-assignment))
