...
A valid `owner/repo` for `MESHLLM_RELEASE_REPOSITORY` makes that fork (e.g. the overlay-hardened Codeflare mesh-llm fork) available as a binary source. The active source defaults to the fork and is switched between fork and official from the Settings runtime panel (`meshllm_release_source` in `router_config`) without a redeploy; invalid values leave upstream as the only source. When the fork is active the agent also aims the launched mesh-llm process at that repository's `native-runtimes.json` (via the `MESH_LLM_NATIVE_RUNTIME_MANIFEST_URL` subprocess variable), so a freshly installing node fetches its platform runtime from the fork instead of mesh-llm's hardcoded upstream default — without which a fork-only tag 404s its native runtime and the node stalls on a permanent runtime error.
