...
1. The gateway playground endpoint forwards the chosen route as `dynamic/<route>` to the selected gateway's compat endpoint. <!-- @impl: packages/router-worker/src/router.ts::handlePlaygroundChat --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-029 playground gateway target forwards dynamic/<route> to the selected gateway compat endpoint) -->
...
7. Both playground endpoints forward an optional OpenAI-format `tools` array and a `max_tokens` cap when the request supplies them, so an agentic request reaches the model verbatim and a runaway response is bounded; when absent, neither field is sent. <!-- @impl: packages/router-worker/src/router.ts::handlePlaygroundChat --> <!-- @impl: packages/router-worker/src/router.ts::handlePlaygroundDirect --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-029 forwards playground tools and a max-token cap to the upstream route when supplied) -->
...
**Intent:** An operator must be able to adjust a model's serving settings — its context window, its model file, how much GPU memory (VRAM) is dedicated to running it, and the per-model MeshLLM runtime tunables (parallel lanes, KV cache types, batch sizes, flash attention, max output tokens, and reasoning) — from the Models drawer, without editing seed configuration, so a machine can be tuned for input caching and throughput and still fit a model within a budget.

**Applies To:** Admin

**Acceptance Criteria:**

1. The model detail drawer exposes an editable context window (blank = Auto), model file, a "Max VRAM (GB, 0 = no limit)" field, and, under an Advanced runtime group, the tunables (parallel lanes, KV cache type for keys and values, prefill batch, micro-batch, flash attention, max output tokens, and reasoning enable/format/budget), each populated from the model's current settings and each carrying plain-language help text and a default hint. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::openModelDrawer --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-021 loads and saves a per-model VRAM budget from the model drawer) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-RUN-002 loads and saves per-model runtime tunables from the model drawer) -->
2. Saving posts to `POST /admin/profiles/config`, which persists the context window, model reference, VRAM budget, and runtime tunables through the validated store path and bumps the profile version so a later default re-seed does not overwrite the edit. <!-- @impl: packages/router-worker/src/router.ts::handleProfileConfig --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-021 configures a profile context window, model ref, and VRAM budget through the validated store path) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RUN-002 persists per-model runtime tunables and clears them back to Auto) -->
3. `POST /admin/profiles/config` accepts a context window of `0` (Auto) and rejects a negative or non-integer context window, a blank model reference, a negative VRAM budget, and an out-of-range tunable (a non-positive parallel/batch/micro-batch/max-output value or an unknown KV cache type); a null / `0` / empty tunable clears the field back to Auto by removing it. An empty or zero VRAM budget means no cap, so the node agent renders no `--max-vram` for that model. <!-- @impl: packages/router-worker/src/router.ts::handleProfileConfig --> <!-- @impl: packages/router-worker/src/router.ts::resolveMeshllmTunables --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-021 configures a profile context window, model ref, and VRAM budget through the validated store path) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RUN-002 persists per-model runtime tunables and clears them back to Auto) -->
