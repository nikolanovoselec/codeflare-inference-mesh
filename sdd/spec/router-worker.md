...
4. Stream failure releases the scheduler reservation and records the node failure signal. <!-- @impl: packages/router-worker/src/scheduler.ts::StoreScheduler.recordFailure --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RTR-003 releases and penalizes stream failures) -->
