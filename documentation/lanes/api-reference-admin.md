...
Direct llama.cpp profiles accept a `llamacpp` block. `parallel` is `-1` (Auto) or `>= 1`; `kvUnified` is boolean, and off with Auto parallel is rejected. GPU layers, cache types, batch sizes, generation cap, flash attention, prompt cache/reuse, reasoning, and bind port use the documented direct-runtime types and ranges. `mmproj: false` disables projector auto-load; `null` restores the default.
