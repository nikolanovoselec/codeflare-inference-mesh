...
Direct llama.cpp metrics also carry the selected/installed version, verified managed backend family, cache settings, slot state, latest cached-token count, and whether the current multimodal model cannot use llama.cpp's cross-divergence reuse optimization. Ordinary text prefix caching remains active through prompt caching. Custom or host binaries whose build is not verified report backend `unknown`; bootstrap failures remain `dependency-missing`. ([REQ-OBS-003](../../sdd/spec/observability.md)) ([REQ-OBS-013](../../sdd/spec/observability.md#req-obs-013-direct-runtime-capability-reporting)) ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap))
