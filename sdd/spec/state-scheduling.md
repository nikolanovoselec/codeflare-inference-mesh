...
4. Session mappings expire after the configured inactivity window. <!-- @impl: packages/router-worker/src/scheduler.ts::reserve --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-004 preserves session affinity when the sticky node remains eligible) -->
