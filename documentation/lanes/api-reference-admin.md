...
Direct llama.cpp profiles accept a `llamacpp` block. `parallel` is `-1` (Auto) or `>= 1`. `kvUnified` is boolean; `null` restores on, while off with Auto parallel is rejected. `gpuLayers` accepts `0`, a positive integer, `"auto"`, or `"all"`; `null` or `""` clears it.
...
`mmproj` is boolean; `false` disables projector auto-load and `null` restores the default. `reasoning` accepts `{ enabled?, format?, budget? }`, while `null` clears the block. `bindPort` is optional and reserved ports are rejected.
