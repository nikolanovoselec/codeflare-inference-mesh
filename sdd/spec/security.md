...
7. Cookie-backed Access admin mutations are accepted only with same-origin browser evidence; cross-site cookie-only writes are rejected. <!-- @impl: packages/router-worker/src/router.ts::hasSameOriginSignal --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SEC-009 rejects cookie-backed admin mutations without same-origin evidence) -->
