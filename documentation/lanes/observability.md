...
Heartbeats also carry `runtimeDetail` — the most recent error-looking line the agent captured from mesh-llm's own stderr (an OOM, a CUDA error, a lane that never came up) — and `nodeState`, the console's raw node state string. Together they make the reason a runtime is wedged legible from the control plane instead of only the host journal; the agent tees mesh-llm's stderr through a bounded ring that keeps the latest error line, and a zero value never overwrites a present one on merge. ([REQ-OBS-011](../../sdd/spec/observability.md))
