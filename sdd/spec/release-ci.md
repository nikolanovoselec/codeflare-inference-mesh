...
3. Manual production deploy refuses to run unless the selected ref is `main`. <!-- @impl: .github/workflows/deploy.yml::REL002ManualIntegrationDeploy --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-002 auto-deploys production only after green main gates and allows manual integration from any branch) -->
...
7. Deploy writes a summary that includes ref, Worker, release tag, environment, and artifact list. <!-- @impl: .github/workflows/deploy.yml::REL002ManualIntegrationDeploy --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-002 auto-deploys production only after green main gates and allows manual integration from any branch) -->
...
2. Deploy creates a checksums file covering every uploaded archive. <!-- @impl: .github/workflows/deploy.yml::REL003ReleaseArtifacts --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-003 builds cross-platform release assets, manifest, optional signature, and GitHub Release) -->
...
4. Workflow safety checks reject unsafe workflow-run checkout patterns, floating runner/action pins, and major-only core action pins. <!-- @impl: .github/workflows/security.yml::REL004SecurityWorkflows --> <!-- @test: packages/router-worker/src/workflows.test.ts (REQ-REL-004 rejects unsafe workflow_run checkout and floating action refs) -->
