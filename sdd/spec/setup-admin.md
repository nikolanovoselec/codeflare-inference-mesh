...
5. The UI holds a responsive layout with no horizontal overflow at any desktop or mobile breakpoint, reflowing multi-column tables to stacked labelled cards and the setup stepper to progress bars named by each step's heading so no label truncates. <!-- @impl: packages/router-worker/src/admin-ui.ts::ADMIN_UI_RESPONSIVE --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-006 serves a responsive browser admin UI for every admin-facing function) -->
