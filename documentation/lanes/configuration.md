...
| `ADMIN_RECOVERY_TOKEN` | n/a | yes only for emergency admin reset | `packages/router-worker/src/auth-gates.ts::resolveGate` | [REQ-ADM-002](../../sdd/spec/setup-admin.md) |
| `SETUP_REOPEN` | n/a | no (break-glass only, set via wrangler) | `packages/router-worker/src/setup-state.ts::breakGlassActive`, `packages/router-worker/src/handlers/setup.ts::handleSetupComplete` | [REQ-ADM-013](../../sdd/spec/setup-admin.md) |
| `NODE_UPSTREAM_TOKEN` | generated during first setup when absent | no | `packages/router-worker/src/inference.ts::resolveUpstreamToken` | [REQ-SEC-001](../../sdd/spec/security.md) |
| `CLOUDFLARE_API_TOKEN_RUNTIME` | n/a | yes for Gateway/domain automation | `packages/router-worker/src/handlers/gateway.ts::syncGatewayForActor`, `packages/router-worker/src/handlers/setup.ts::handleCustomDomain`, `packages/router-worker/src/handlers/setup.ts::handleSetupAccess`, `packages/router-worker/src/handlers/setup.ts::handleZones`, `packages/router-worker/src/handlers/gateway.ts::handleGatewayOptions` | [REQ-GWY-003](../../sdd/spec/gateway.md), [REQ-ADM-005](../../sdd/spec/setup-admin.md), [REQ-ADM-012](../../sdd/spec/setup-admin.md), [REQ-GWY-005](../../sdd/spec/gateway.md) |
| `CLOUDFLARE_ACCOUNT_ID` | n/a | yes for Gateway/domain automation | `packages/router-worker/src/handlers/gateway.ts::syncGatewayForActor`, `packages/router-worker/src/handlers/setup.ts::handleCustomDomain`, `packages/router-worker/src/handlers/setup.ts::handleSetupAccess`, `packages/router-worker/src/handlers/setup.ts::handleZones`, `packages/router-worker/src/handlers/gateway.ts::handleGatewayOptions` | [REQ-GWY-003](../../sdd/spec/gateway.md), [REQ-ADM-005](../../sdd/spec/setup-admin.md), [REQ-ADM-012](../../sdd/spec/setup-admin.md), [REQ-GWY-005](../../sdd/spec/gateway.md) |
...
| `SESSION_AFFINITY_KEY` | `ADMIN_TOKEN` only in local/test fallback | yes for deploy | `packages/router-worker/src/inference.ts::directAffinitySecret` | [REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity) |
...
`SESSION_AFFINITY_KEY` is the HMAC key for direct llama.cpp `body.user` values. D1 stores only hashed user/session ids in `direct_sessions`; raw ids never persist. Local tests can fall back to `ADMIN_TOKEN`, but production should set a distinct secret. ([REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity)) <!-- @impl: packages/router-worker/src/inference.ts::directAffinitySecret --> <!-- @impl: packages/router-worker/src/inference.ts::hmacHex -->

## Worker vars

| Variable | Default | Required | Consumed by | Implements |
| --- | --- | --- | --- | --- |
| `AI_GATEWAY_ACCOUNT_ID` | `set-by-deploy-or-runtime-secret` | no | `packages/router-worker/src/handlers/gateway.ts::syncGatewayForActor`, `packages/router-worker/src/handlers/setup.ts::handleCustomDomain` | [REQ-GWY-003](../../sdd/spec/gateway.md), [REQ-ADM-005](../../sdd/spec/setup-admin.md) |
| `MAX_REQUEST_BYTES` | `16777216` | no | `packages/router-worker/src/handlers/inference.ts::handleChat` | [REQ-RTR-002](../../sdd/spec/router-worker.md) |
...
| `AI_GATEWAY_ID` | `inference-mesh` | no | `packages/router-worker/src/handlers/gateway.ts::gatewaySettings` | [REQ-GWY-003](../../sdd/spec/gateway.md) |
| `AI_GATEWAY_ROUTE_NAME` | `codeflare-mesh` | no | Declared in Wrangler but not consumed; the default mesh's dynamic route name is pinned to the stable public model `codeflare-mesh` in `packages/router-worker/src/handlers/gateway.ts::gatewaySettings`, and non-default meshes' route names derive from the mesh registry (`codeflare-mesh-<id>`). | [REQ-GWY-003](../../sdd/spec/gateway.md) |
| `AI_GATEWAY_PROVIDER_NAME` | `codeflare-inference-mesh` | no | `packages/router-worker/src/handlers/gateway.ts::gatewaySettings` | [REQ-GWY-003](../../sdd/spec/gateway.md) |
| `AI_GATEWAY_PUBLIC_MODEL` | `codeflare-mesh` | no | Declared in Wrangler but not consumed; the forwarded public model is pinned to the stable public model `codeflare-mesh` in `packages/router-worker/src/handlers/gateway.ts::gatewaySettings`. | [REQ-GWY-003](../../sdd/spec/gateway.md) |
| `WORKER_BASE_URL` | optional bootstrap origin override | no; custom-domain setup and installers use the request origin when unset or still a placeholder; Gateway sync only uses it to distinguish stored explicit overrides | `packages/router-worker/src/handlers/setup.ts::handleCustomDomain`, `packages/router-worker/src/handlers/gateway.ts::syncGatewayForActor`, `packages/router-worker/src/handlers/setup.ts::handleInstaller` | [REQ-ADM-004](../../sdd/spec/setup-admin.md), [REQ-ADM-005](../../sdd/spec/setup-admin.md), [REQ-GWY-003](../../sdd/spec/gateway.md#req-gwy-003-dynamic-route-automation), [REQ-REL-005](../../sdd/spec/release-ci.md#req-rel-005-deploy-execution-safety) |
| `WORKER_NAME` | `codeflare-inference-mesh-router` (`codeflare-inference-mesh-router-integration` in integration) | no | `packages/router-worker/src/handlers/setup.ts::handleCustomDomain`, `packages/router-worker/src/handlers/setup.ts::handleSetupAccess` | [REQ-ADM-005](../../sdd/spec/setup-admin.md), [REQ-ADM-012](../../sdd/spec/setup-admin.md) |
| `AGENT_RELEASE_TAG` | `agent-release-tag-placeholder` | set by deploy for real installers | `packages/router-worker/src/handlers/setup.ts::handleInstallScript` | [REQ-REL-003](../../sdd/spec/release-ci.md) |
| `GITHUB_REPOSITORY` | `nikolanovoselec/codeflare-inference-mesh` | yes for installers and agent-version listing | `packages/router-worker/src/handlers/setup.ts::handleInstaller`, `packages/router-worker/src/handlers/setup.ts::handleInstallScript`, `packages/router-worker/src/agent-versions.ts::handleAgentVersionsList` | [REQ-ADM-004](../../sdd/spec/setup-admin.md), [REQ-REL-003](../../sdd/spec/release-ci.md), [REQ-ADM-008](../../sdd/spec/setup-admin.md) |
| `MESHLLM_RELEASE_REPOSITORY` | unset (upstream `Mesh-LLM/mesh-llm` only) | no — see note below | `packages/router-worker/src/runtime-versions.ts::meshllmReleaseRepository`, `packages/router-worker/src/runtime-versions.ts::activeMeshllmRepository`, `packages/node-agent/internal/agent/meshllm_install.go::EnsureMeshLLMVersion` | [REQ-NODE-014](../../sdd/spec/node-agent.md#req-node-014-configurable-runtime-release-source) |

A valid `owner/repo` for `MESHLLM_RELEASE_REPOSITORY` makes that fork (e.g. the overlay-hardened Codeflare mesh-llm fork) available as a binary source. The active source defaults to the fork and can switch between fork and official from Settings without a redeploy; invalid values leave upstream as the only source.

When the fork is active, the agent points the launched process at that repository's native-runtime manifest. Fresh nodes therefore fetch the platform runtime from the same source as the binary instead of failing on an unavailable upstream tag. ([REQ-NODE-014](../../sdd/spec/node-agent.md#req-node-014-configurable-runtime-release-source)) <!-- @impl: packages/node-agent/internal/agent/meshllm_render.go::meshLLMNativeRuntimeManifestURL -->
...
| `llamaCppBinaryPath` | empty (managed upstream release asset) | no | `packages/node-agent/cmd/inference-mesh-agent/runtime_launch.go::llamaCppBinaryPath`, `packages/node-agent/internal/agent/llamacpp_manager.go::LlamaCppManager` | [REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap) |
| `INFERENCE_MESH_LLAMA_CPP_BACKEND` | auto-detected (`metal` on macOS; `rocm`, `nvidia`, `vulkan`, else `cpu`) | no | `packages/node-agent/internal/agent/llamacpp_install.go::DetectLlamaCppBackend`, `packages/node-agent/internal/agent/llamacpp_install.go::ResolvedLlamaCppBackend` | [REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap) |
...
For direct llama.cpp, `DetectLlamaCppBackend` selects the requested managed backend and `ResolvedLlamaCppBackend` identifies the family in the chosen release archive. The agent first tries a host-installed `llama-server` before downloading that managed asset. `INFERENCE_MESH_LLAMA_CPP_BACKEND` can force the managed selector when auto-detection is wrong. ([REQ-NODE-016](../../sdd/spec/node-agent.md#req-node-016-direct-runtime-backend-identity))

`llamaCppBinaryPath` is the escape hatch when the service account cannot see a known-good CUDA-enabled install. The agent skips managed installation and runs that binary with the same profile flags; because its build is not verified by the installer, backend telemetry reports `unknown`. ([REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap)) ([REQ-NODE-016](../../sdd/spec/node-agent.md#req-node-016-direct-runtime-backend-identity))
...
The router calls `seedDefaultProfiles(DEFAULT_MODEL_PROFILES)` at request entry. The shipped catalog is exactly one starter profile — `mesh-smoke-qwen25-1.5b` (Qwen2.5 Coder 1.5B) — seeded exactly once: the `default_profiles_seeded` router-config marker gates the write, so stored rows are never refreshed or retired afterwards and a deleted starter never re-seeds. When an active profile already owns a starter alias at first seed, the starter arrives inactive so the alias keeps a single active owner. Deployments that seeded earlier catalogs keep those rows as ordinary deletable profiles. ([REQ-RUN-002](../../sdd/spec/runtime-profiles.md#req-run-002-default-model-profiles)) ([REQ-RUN-009](../../sdd/spec/runtime-profiles.md#req-run-009-profile-seeding-and-activation-exclusivity)) <!-- @impl: packages/router-worker/src/router.ts::createRouter --> <!-- @impl: packages/router-worker/src/store.ts::seedDefaultProfiles --> <!-- @impl: packages/router-worker/src/store.ts::DEFAULT_PROFILES_SEEDED_KEY -->
...
Context window defaults to Auto so MeshLLM sizes it to the GPU; pinning a small context on a large model collapses the lane count. Parallel lanes and the input cache default on (not Auto): an omitted parallel lets MeshLLM pick a single lane, and an omitted cache defers to family auto-detection that leaves it off for uncertified families, which together is why input caching never engaged.

Direct llama.cpp defaults target high-throughput, cache-local coding sessions: context window Auto (llama-server loads the model's native training context), parallel slots Auto, unified KV on, KV cache types `q4_0`, prefill batch `8192`, micro-batch `2048`, flash attention on, prompt cache on, cache reuse `256`, generation cap `16384`, and reasoning on / `deepseek` / `8192`. Raising context, parallelism, batch, or micro-batch can improve capacity or prompt-loading speed, but it also increases memory pressure; lower those values if requests fail under load. Keep unified KV on: without it llama-server divides the context window evenly across slots, so an explicit slot count silently caps every request at context ÷ slots and long coding-agent requests fail with 400.

Direct llama.cpp profiles use these settings:

| Setting | llama-server flag | Default | What it is / does |
| --- | --- | --- | --- |
| Context window | `--ctx-size` | Auto (`0`) | Auto renders `--ctx-size 0`, loading the model's native training context. Pin a value (`>= 4096`) to cap the KV budget; larger uses more GPU memory. |
| Parallel slots | `--parallel` | Auto (`-1`) | Number of concurrent llama.cpp slots for the node-local runtime. Auto lets llama-server plan the slot count (4) with unified KV; an explicit count keeps the full per-request context only while unified KV stays on. |
| Unified KV | `--kv-unified` / `--no-kv-unified` | On | Shares one KV buffer across all slots so a single request can use the entire context window. Off divides the context evenly per slot (context ÷ slots per request); Off with Auto slots is rejected because llama-server force-enables unified KV under Auto. |
...
| Cache reuse | `--cache-reuse` | `256` | Cross-divergence KV reuse optimization. Multimodal models may reject it; ordinary text prefix caching remains available when prompt caching is enabled. |
