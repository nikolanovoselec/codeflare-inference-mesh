...
9. Admin observes the mesh on the dashboard — topology, drawers, stats, sortable nodes table, and a tokens-per-second trace on five-second polling — and verifies inference from the playground. ([REQ-OBS-002](../../sdd/spec/observability.md)) ([REQ-OBS-007](../../sdd/spec/observability.md)) ([REQ-OBS-010](../../sdd/spec/observability.md)) ([REQ-ADM-015](../../sdd/spec/setup-admin.md)) ([REQ-ADM-016](../../sdd/spec/setup-admin.md)) ([REQ-ADM-007](../../sdd/spec/setup-admin.md))
...
11. Admin can add a model beyond the seeded default profiles from the dashboard's add-model form or directly via `POST /admin/profiles/add`, supplying a mesh-llm-compatible model reference and a serving mode (single-machine or split); the new profile joins the catalog inactive with zero rollout and reaches production only through the existing rollout/activation control plane. ([REQ-RUN-011](../../sdd/spec/runtime-profiles.md#req-run-011-custom-model-onboarding)) ([REQ-ADM-025](../../sdd/spec/setup-admin.md#req-adm-025-add-a-model-console-control))
