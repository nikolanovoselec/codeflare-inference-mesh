...
1. The Admin picks a Gateway and route from lists discovered through the runtime token, or names a new gateway/route to create; the account ID, Worker URL, provider name, and public model are resolved server-side and never entered by hand. <!-- @impl: packages/router-worker/src/admin-ui.ts::adminUiHtml --> <!-- @impl: packages/router-worker/src/router.ts::handleGatewaySync --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 connects a gateway from Routing using discovered or new names only) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 automates provider, route, version, and deployment creation while leaving BYOK manual) -->
