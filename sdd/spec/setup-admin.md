...
4. Admin status and the node detail drawer expose each node's runtime binary status: runtime kind, desired version, installed version when known, `pending`/`installing`/`installed`/`failed` state, and install error when present; the console chip always leads with the runtime's name (`llama.cpp b9928`, `meshllm 0.72.2`), never a bare version. <!-- @impl: packages/router-worker/src/router.ts::runtimeBinaryStatus --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::openNodeDrawer --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-012 renders runtime install status in the node table and drawer) -->
...
8. A mesh selector on the topology filters the rendered machines (and the caption counts) to one machine group, defaults to all meshes, keeps the operator's selection across status polls, and falls back to all when the selected mesh is deleted. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::syncTopoMeshSelect --> <!-- @impl: packages/router-worker/src/admin-ui-contract.ts::ADMIN_UI_TOPOLOGY --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 overview topology filters machines to the selected mesh and survives the poll) -->

9. The Overview "Mesh status" panel renders a responsive grid of per-mesh dashboard cards, each edged in its status tone (green when serving, amber when a deployed model has no serving machine, neutral for a mesh with no model) and structured as: the mesh name in the mesh vocabulary color paired with the mesh's callable route chip (the tone edge alone carries state — no status word or dot renders), the active model's display name over its mono model file reference and its own row of provider and serving-mode pills (or a no-model note with no pills), a machines/serving metric strip whose third cell shows the model's last stored Speed Test as whole-number prompt / generation tok/s (dash before a run exists), and a serving-capacity track filled to the served fraction — all carried as `data-state`/`data-state-tone`/`data-machines`/`data-serving`/`data-speed-prompt`/`data-speed-gen`/`data-fill` contract attributes. A machine counts as serving only when it is online and not deactivated, has adopted the profile, and reports its model ready — a deactivated or offline machine's stale adopted/ready record never counts. The Overview carries no activity feed (the activity log lives in Settings). <!-- @impl: packages/router-worker/src/admin-ui-client.ts::renderMeshStatus --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::routeChipEl --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::profilePills --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 overview mesh status cards summarize each mesh: model, machines, serving, last speed test) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 does not fabricate a mesh-card speed test before one is reported) -->
...
1. Each machine's status renders as one word from the fixed operator vocabulary, derived once by the router and carried on every node projection (`displayStatus`) so the console and the API never disagree: `Serving` (green) while a model or split stage is actually served, `Preparing` (yellow) while online and starting/downloading/loading, `Disconnected` (yellow) while heartbeating with no reachable runtime, `Offline` (red, with a last-seen age when known) when it stops checking in, and `Error` (red) on a runtime failure or missing dependency — plus the `Deactivated`/`Removed`/`Draining` lifecycle labels. Role and work detail (stage ownership, loading step) stay in data attributes and the drawer, never in the visible label. <!-- @impl: packages/router-worker/src/router.ts::nodeDisplayStatus --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::ADMIN_UI_CLIENT_SCRIPT --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-ADM-020 REQ-API-004 node projections carry the derived operator status vocabulary) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 shows a plain node status and never the stale runtime substate when offline) -->
...
4. The Nodes section filters the table by a status chip — labeled All, Serving, Not serving, Offline over the `all`/`ready`/`active`/`offline` filter keys — and by a search box that applies once at least three characters are typed, matching a machine's id or name; the section legend explains the five status words. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::ADMIN_UI_CLIENT_SCRIPT --> <!-- @impl: packages/router-worker/src/admin-ui-views.ts::nodesSection --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 filters the nodes table by status chip and by search) -->
...
8. The gateway card's route line lists every ensured dynamic route — one per mesh (REQ-GWY-009) — falling back to the single stored route name for a pre-mesh sync result. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::gatewayRouteNames --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-024 REQ-GWY-009 the gateway card lists every ensured mesh route) -->
