...
9. The Overview "Mesh status" panel renders one card per machine group merging its identity and live state in the shared pill vocabulary: the purple mesh pill with the status word beside it (green when serving, amber when a deployed model has no serving machine, neutral for a mesh with no model) and the right-aligned callable route chip; the active model's display name with its provider and serving-mode pills (or a no-model note with no pills); and machine/serving counts with live aggregate tok/s while serving — all carried as `data-state`/`data-state-tone`/`data-machines`/`data-serving`/`data-toks` contract attributes; the Overview carries no activity feed (the activity log lives in Settings). <!-- @impl: packages/router-worker/src/admin-ui-client.ts::renderMeshStatus --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::routeChipEl --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::profilePills --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 overview mesh status rows summarize each mesh: model, machines, serving, throughput) -->
