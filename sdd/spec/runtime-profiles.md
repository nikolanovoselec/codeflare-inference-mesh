...
2. Rendered launch arguments enable unified KV (`--kv-unified`) unless the profile explicitly disables it (`--no-kv-unified`), and Auto parallel passes through as `--parallel -1`, so a single request can use the full context window instead of a per-slot share. <!-- @impl: packages/node-agent/internal/agent/llamacpp_manager.go::RenderLlamaCppArgs --> <!-- @test: packages/node-agent/internal/agent/llamacpp_manager_test.go (TestREQRUN015LlamaCppRenderArgsKVUnifiedAndAutoParallel) -->
