...
| Input (prefix) cache | `model_fit.prefix_cache.enabled` / `.max_entries` | On, `16` entries | Reuses the KV of a shared prompt prefix so a follow-up turn prefills only the new tokens; this is what populates `prompt_tokens_details.cached_tokens`. It is NOT enabled by parallel lanes: without an explicit enable MeshLLM defers to family auto-detection, which leaves the cache off for uncertified model families. `max_entries` is capped low because the uncertified fallback of `128` overruns the KV cell pool (HTTP 502 `llama_decode failed`). Needs parallel lanes `2` or more. |
