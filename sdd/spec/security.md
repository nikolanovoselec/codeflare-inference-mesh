...
7. A revoked node is treated as gone: it never appears in any fleet listing (the console status view or the automation node list) and node-configuration requests refuse it as unknown, even when a mid-revoke failure leaves its credential-stripped row behind. <!-- @impl: packages/router-worker/src/store.ts::listNodes --> <!-- @impl: packages/router-worker/src/router.ts::handleApiNodeReconfigure --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SEC-002 hides a revoked tombstone node from every fleet listing) --> <!-- @test: packages/router-worker/src/store.test.ts (REQ-SEC-002 listNodes excludes revoked tombstone rows that getNode can still reach) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SEC-002 refuses to reconfigure a revoked tombstone node) -->
