...
4. D1 stores node records, model profiles, public aliases, and audit events; node and model records carry the mesh membership field, records stored before machine groups existed read back as members of the default mesh, and the schema carries no session-lease or reservation tables because the request path forwards statelessly. <!-- @impl: packages/router-worker/src/store.ts::STORE_ANCHORS --> <!-- @impl: packages/router-worker/src/profiles.ts::normalizeModelProfile --> <!-- @impl: packages/router-worker/migrations/0004_drop_dead_session_tables.sql::SCH001DeadSessionSchemaDrop --> <!-- @test: packages/router-worker/src/store.test.ts (REQ-SCH-001 REQ-SCH-002 persists durable router state and reselects the eligible node from D1) --> <!-- @test: packages/router-worker/src/store.test.ts (REQ-RUN-016 coerces stored profiles and nodes without meshId to the default mesh) --> <!-- @test: packages/router-worker/src/migrations.test.ts (REQ-SCH-001 applies the migration sequence to exactly the durable tables, with no session-lease or reservation schema) -->
