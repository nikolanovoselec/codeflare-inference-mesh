...
5. A runtime that fails with `mesh-llm readiness deadline exceeded` is treated as restart-needed on the next heartbeat create directive, so a node self-heals instead of requiring Force Reload. <!-- @impl: packages/node-agent/internal/agent/meshllm_manager.go::MeshLLMManager.NeedsRestart --> <!-- @test: packages/node-agent/internal/agent/meshllm_manager_test.go (TestREQRUN005ReadinessDeadlineFailureNeedsRestart) -->
