...
Each inference node must run Cloudflare One Client / WARP enrolled into the same account network as the Worker VPC binding. The node agent detects the WARP adapter by name (`CloudflareWARP`) and by the WARP CGNAT range `100.96.0.0/12`, so headless server enrollments (the WARP Connector / `warp-cli` connector mode) are detected the same way as desktop WARP, and it advertises that WARP IP plus its Mesh-facing inference port. The router stores that `IP:PORT` and calls it through `env.MESH.fetch(...)`; it does not call public node URLs. ([REQ-NODE-001](../../sdd/spec/node-agent.md)) ([REQ-NODE-008](../../sdd/spec/node-agent.md)) ([REQ-RTR-004](../../sdd/spec/router-worker.md))
...
| `dataDir` | `.inference-mesh` (installer sets `/var/lib/inference-mesh`) | no | `packages/node-agent/cmd/inference-mesh-agent/main.go::defaultDataDir` | [REQ-RUN-003](../../sdd/spec/runtime-profiles.md) |
...
The `INFERENCE_MESH_CONFIG` environment variable overrides the config path for both the `install` and `run` commands. The Unix installer sets it (and passes `--config`) to `/var/lib/inference-mesh/config.json` and points the systemd unit's `WorkingDirectory` and `Environment` at the same system state directory, so the running service resolves the exact config the install step wrote regardless of the invoking user's home directory. ([REQ-NODE-001](../../sdd/spec/node-agent.md)) ([REQ-ADM-004](../../sdd/spec/setup-admin.md)) <!-- @impl: packages/node-agent/internal/agent/config.go::ConfigPath --> <!-- @impl: packages/router-worker/src/installers.ts::INSTALLER_ANCHORS -->
