...
1. Heartbeat metrics include runtime state, loaded model, active profile ID/version, in-flight requests, and last request duration. <!-- @impl: packages/node-agent/internal/agent/metrics.go::MetricsAnchors --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-NODE-002 REQ-OBS-003 accepts authenticated heartbeats and stores node metrics) -->
2. Heartbeat metrics include prompt throughput, generation throughput, and last runtime error when the runtime reports them. <!-- @impl: packages/node-agent/internal/agent/metrics.go::MetricsAnchors --> <!-- @test: packages/node-agent/internal/agent/agent_test.go (TestREQRUN003RuntimeDependencyMissingState) -->
...
2. Mid-stream node failures release reservations and increase the node's recent failure score. <!-- @impl: packages/router-worker/src/scheduler.ts::StoreScheduler.recordFailure --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-RTR-002 releases a reservation when Mesh fetch throws) -->
3. Invalid Mesh connection data makes the node ineligible for scheduling. <!-- @impl: packages/router-worker/src/scheduler.ts::isEligible --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-003 REQ-OBS-004 excludes expired unhealthy and unsafe nodes from scheduling) -->
4. Capacity exhaustion is reported as no-node when no other node is eligible. <!-- @impl: packages/router-worker/src/scheduler.ts::reserve --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-003 REQ-OBS-004 excludes expired unhealthy and unsafe nodes from scheduling) -->
