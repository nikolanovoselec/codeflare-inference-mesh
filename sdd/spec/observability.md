...
6. The mesh panel titles each entry by the model's display name and leads with a plain-language sharing summary (machine count and whether it is ready), keeping the coordinator, peers, rotation, and secret fields behind a technical-details disclosure. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::ADMIN_UI_CLIENT_SCRIPT --> <!-- @test: packages/router-worker/src/admin-ui-mesh.test.ts (REQ-OBS-007 shows a plain sharing summary while keeping the technical fields behind a disclosure) -->
