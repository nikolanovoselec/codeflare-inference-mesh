...
**Verification:** The credential-boundary router test checks cross-family credential rejection; `TestREQNODE004DashboardRuntimeControlsUseController` checks dashboard-token enforcement. <!-- @impl: packages/router-worker/src/router-routing.test.ts::CredentialBoundaryTestAnchor --> <!-- @impl: packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE004DashboardRuntimeControlsUseController -->

**Implements:** [REQ-SEC-001](../../sdd/spec/security.md)

| Boundary | Credential | Purpose | REQs |
| --- | --- | --- | --- |
| Client to AI Gateway | AI Gateway token with the `AI Gateway: Run` scope (`cf-aig-authorization`) | Authenticated Gateway rejects requests without a valid AI Gateway token carrying the `AI Gateway: Run` scope, so the open gateway cannot forward the stored provider key on a stranger's behalf (a token that lacks `Run` surfaces as "Model execution failed"; see [troubleshooting.md](troubleshooting.md#clients-get-model-execution-failed-from-the-dynamic-route-while-the-playground-works)). | [REQ-SEC-012](../../sdd/spec/security.md), [REQ-SEC-001](../../sdd/spec/security.md) |
...
**Mitigation:** The managed MeshLLM runtime runs behind the node proxy. The Mesh-facing listener requires the upstream token. The MeshLLM console API (default port `3131`) binds to localhost only and carries the mesh invite token, so it is never exposed or proxied; the agent reads it locally and reports the token only inside authenticated heartbeats. The managed vLLM server — which binds all interfaces by default upstream and has no authentication of its own — is rendered with `--host 127.0.0.1` unconditionally, so only the authenticated node proxy reaches it. Dashboard runtime controls are localhost-only, require the dashboard token, and reject browser Origin headers that do not match the dashboard origin.

**Verification:** `packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE004DashboardRuntimeControlsUseController` asserts dashboard-token enforcement; `packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE003UpstreamProxyEnforcesBearerAndStreams` asserts upstream proxy auth and header filtering; `TestREQRUN006PollStatusCapturesTokenAndMeshID` asserts the invite token is captured from the localhost console status poll, never from process stdout; `packages/node-agent/internal/agent/vllm_manager_test.go::TestREQSEC013VllmRenderArgsAlwaysBindsLoopback` asserts the vLLM argv binds loopback regardless of profile settings. <!-- @impl: packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE004DashboardRuntimeControlsUseController --> <!-- @impl: packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE003UpstreamProxyEnforcesBearerAndStreams --> <!-- @impl: packages/node-agent/internal/agent/meshllm_manager_test.go::TestREQRUN006PollStatusCapturesTokenAndMeshID --> <!-- @impl: packages/node-agent/internal/agent/vllm_manager_test.go::TestREQSEC013VllmRenderArgsAlwaysBindsLoopback -->

**Implements:** [REQ-SEC-004](../../sdd/spec/security.md), [REQ-SEC-013](../../sdd/spec/security.md#req-sec-013-vllm-api-exposure), [REQ-SEC-008](../../sdd/spec/security.md), [REQ-NODE-004](../../sdd/spec/node-agent.md), [REQ-RUN-006](../../sdd/spec/runtime-profiles.md)
...
**Verification:** `packages/router-worker/src/router-admin-console.test.ts` (`AdminConfigurationUiTestAnchor`) asserts the `content-security-policy` and `x-frame-options` header values on the admin response, and `packages/router-worker/src/admin-ui-mesh.test.ts` (`REQ-ADM-006 verifies the admin token before storing it`) asserts the verify-before-store order against the executed client script, and `packages/router-worker/src/admin-ui-mesh.test.ts` (`REQ-ADM-006 signs out and clears the stored admin token`) asserts sign-out removes the token from both storage locations and returns to the login view; no automated test currently asserts an admin-UI output-encoding boundary, so that narrower gap remains audit pending. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::ADMIN_UI_CLIENT_SCRIPT --> <!-- @impl: packages/router-worker/src/router-admin-console.test.ts::AdminConfigurationUiTestAnchor -->
...
**Mitigation:** The trust anchor for agent updates is the GitHub repository and its release process, with an operator in the loop: the desired agent version is only ever an admin-selected tag from the validated release list, distributed via heartbeat, and the agent stages a downloaded binary only after its SHA-256 matches the release's `checksums.txt`; any failure leaves the current version running. Runtime binaries are also operator-selected release tags: MeshLLM defaults keep embedded per-asset checksums, selected MeshLLM versions verify the upstream checksum sidecar, and selected llama.cpp versions verify the GitHub release asset digest before installing `llama-server`. The deploy pipeline additionally signs `checksums.txt` with cosign when `COSIGN_PRIVATE_KEY` is configured; the signature is an out-of-band operator verification artifact, and the agent does not verify it during self-update.

**Verification:** `TestREQNODE005StagesSelfUpdateOnlyWhenChecksumMatches` asserts checksum-gated staging; `TestREQNODE009FailureReportsLastErrorAndKeepsCurrentVersion` asserts failed updates leave the running version; the agent-version and runtime-version selection tests assert only listed release tags are accepted; runtime installer tests assert checksum/digest mismatch prevents installation. <!-- @impl: packages/node-agent/internal/agent/selfupdate.go::SelfUpdateAnchors --> <!-- @impl: packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE005StagesSelfUpdateOnlyWhenChecksumMatches --> <!-- @impl: packages/router-worker/src/agent-versions.ts::AGENT_VERSIONS_ANCHORS --> <!-- @impl: packages/router-worker/src/runtime-versions.ts::RUNTIME_VERSIONS_ANCHORS --> <!-- @impl: packages/node-agent/internal/agent/llamacpp_install.go::LlamaCppInstallAnchors -->

**Implements:** [REQ-NODE-005](../../sdd/spec/node-agent.md), [REQ-NODE-009](../../sdd/spec/node-agent.md), [REQ-NODE-013](../../sdd/spec/node-agent.md#req-node-013-runtime-binary-bootstrap), [REQ-REL-003](../../sdd/spec/release-ci.md), [REQ-ADM-008](../../sdd/spec/setup-admin.md), [REQ-ADM-033](../../sdd/spec/setup-admin.md#req-adm-033-runtime-binary-version-and-install-visibility)
...
| Console role resolution | [security.md](../../sdd/spec/security.md) | `packages/router-worker/src/auth-gates.ts::resolveRole` <!-- @impl: packages/router-worker/src/auth-gates.ts::resolveRole --> |
...
| vLLM loopback bind | [security.md](../../sdd/spec/security.md#req-sec-013-vllm-api-exposure) | `packages/node-agent/internal/agent/vllm_manager.go::RenderVllmArgs` <!-- @impl: packages/node-agent/internal/agent/vllm_manager.go::RenderVllmArgs --> |
