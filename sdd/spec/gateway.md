...
4. Re-running sync reuses the existing matching provider and route without creating a new route version or deployment, except that a matching route left disabled out of band is re-enabled, accepting a new version and deployment, so sync never leaves it silently non-serving. <!-- @impl: packages/router-worker/src/cloudflare-api.ts::syncCustomProvider --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 reuses existing Cloudflare Gateway resources on repeat sync) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 re-enables a disabled route even when its routing elements already match) -->
