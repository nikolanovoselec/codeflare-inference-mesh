...
7. Mesh health entries carry member-reported split-readiness blockers such as `split_capacity_shortfall`, and the Admin UI renders them as needs-attention with capacity and participant detail instead of marking standby API clients green. <!-- @impl: packages/router-worker/src/mesh-state.ts::meshHealth --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::buildMeshCard --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::nodeStatusText --> <!-- @test: packages/router-worker/src/mesh-state.test.ts (REQ-OBS-007 carries split readiness blockers into mesh health) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-007 surfaces split capacity shortfall instead of marking standby API clients green) --> <!-- @test: packages/router-worker/src/admin-ui-mesh.test.ts (REQ-OBS-007 renders split capacity shortfall in mesh technical details) -->
...
3. The console node table and Manage drawer surface these diagnostics: the table carries `nodeState` as a status-detail contract value, labels healthy split roles such as API client or serving peer without treating yellow role state as failure, replaces a split profile stuck at standby with no peers/stages with a mesh peer-discovery blocker, and the drawer renders it as a read-only row alongside the captured runtime error while the runtime is not ready/running, mesh role, peer count, stage count (only when split), API/console reachability, and the mesh-llm version; absent heartbeat fields and stale non-fatal stderr warnings on healthy ready/running nodes render as not reported or are suppressed, not as `down`, `0`, `on`, or current install errors. <!-- @impl: packages/router-worker/src/admin-ui-client.ts::renderNodesTable --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::openNodeDrawer --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 shows a plain node status and never the stale runtime substate when offline) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-011 the node drawer surfaces the runtime error, node state, and mesh diagnostics) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-011 direct node drawer does not turn unreported heartbeat fields into failures) --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-OBS-011 surfaces split mesh peer discovery blockers without SSH) -->
