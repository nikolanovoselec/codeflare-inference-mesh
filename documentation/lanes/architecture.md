...
| Request dispatcher | Rate limiting, default-profile seeding, the custom-domain host gate, credential enforcement, and error mapping; then one dispatch over the route table. Owns no route's logic. | `packages/router-worker/src/router.ts` | [REQ-RTR-001](../../sdd/spec/router-worker.md#req-rtr-001-route-family-separation), [REQ-RTR-006](../../sdd/spec/router-worker.md#req-rtr-006-declared-credential-gates) |
| Route table | Ordered list of every route with its method, path, and required credential class; first match wins. | `packages/router-worker/src/routes.ts` | [REQ-RTR-006](../../sdd/spec/router-worker.md#req-rtr-006-declared-credential-gates) |
| Route handlers | One module per route family: the data plane, the node protocol, fleet status, setup, gateway, nodes, meshes, models, keys, playground, and versions. A handler receives an already-authenticated caller and never repeats its route's credential check. | `packages/router-worker/src/handlers/` | [REQ-RTR-001](../../sdd/spec/router-worker.md) |
| Console script fragments | The browser console script, one fragment per topic, concatenated verbatim into a single IIFE. Fragments are not standalone modules: each begins where the previous ended, so their order is load-bearing, and none may interpolate. Console actions dispatch through a table keyed by action id. | `packages/router-worker/src/client/` | [REQ-ADM-007](../../sdd/spec/setup-admin.md#req-adm-007-operator-dashboard) |
...
| HTTP primitives | Response builders and request-body readers shared by every handler; carries the request-id and framing headers. | `packages/router-worker/src/http.ts` | [REQ-OBS-001](../../sdd/spec/observability.md) |
| Profile configuration | Validates and normalises model-profile and node configuration bodies into records or errors, holding no HTTP concern. | `packages/router-worker/src/profile-config.ts` | [REQ-RUN-013](../../sdd/spec/runtime-profiles.md), [REQ-RUN-014](../../sdd/spec/runtime-profiles.md) |
| Data plane | Node selection, body rewriting, Mesh forwarding, and the direct-session affinity that picks a cache-warm node for direct profiles (llama.cpp and vLLM). | `packages/router-worker/src/inference.ts` | [REQ-RTR-002](../../sdd/spec/router-worker.md), [REQ-SCH-002](../../sdd/spec/state-scheduling.md) |
| Speed test | Measures one streamed request through the real forward path under a bounded deadline; an admin or automation run stores the result per resolved profile unless the run timed out, since a partial read is never written, and a read-only viewer's run never stores. | `packages/router-worker/src/speed-test.ts` | [REQ-ADM-034](../../sdd/spec/setup-admin.md#req-adm-034-direct-router-speed-test) |
...
| Runtime versions | MeshLLM/llama.cpp/vLLM release-tag caches, fleet-wide desired runtime versions, and heartbeat distribution. | `packages/router-worker/src/runtime-versions.ts` | [REQ-ADM-033](../../sdd/spec/setup-admin.md#req-adm-033-runtime-binary-version-and-install-visibility), [REQ-RUN-025](../../sdd/spec/runtime-profiles.md#req-run-025-console-vllm-surface) |
...
Router configuration records in D1 include the agent release-tag cache (`agent_versions_cache`) and selected fleet version (`desired_agent_version`); the runtime release caches and selections (`meshllm_versions_cache`, `llamacpp_versions_cache`, `vllm_versions_cache`, `desired_meshllm_version`, `desired_llamacpp_version`, `desired_vllm_version`); the mesh registry (`meshes`); and the starter-seeding marker (`default_profiles_seeded`). ([REQ-SCH-001](../../sdd/spec/state-scheduling.md))
...
- All three managed runtimes launch through one seam: a per-kind `{start, restart}` spec table behind the `RuntimeManager` interface. ([REQ-RUN-024](../../sdd/spec/runtime-profiles.md#req-run-024-managed-vllm-node-lifecycle))

Direct vLLM profiles are capability-gated to Linux + NVIDIA CUDA hosts, serve a Hugging Face safetensors repository on a loopback-only OpenAI server, and route through the same direct session affinity as llama.cpp. Provisioning installs into a pinned per-version virtual environment under the data directory: `uv venv` with the pinned Python, then `uv pip install vllm==<version> --torch-backend=auto`, with the completion marker gating reuse. ([REQ-RUN-021](../../sdd/spec/runtime-profiles.md#req-run-021-direct-vllm-custom-profiles)) ([REQ-RUN-022](../../sdd/spec/runtime-profiles.md#req-run-022-direct-vllm-launch-rendering)) ([REQ-NODE-017](../../sdd/spec/node-agent.md#req-node-017-vllm-runtime-bootstrap)) ([REQ-SEC-013](../../sdd/spec/security.md#req-sec-013-vllm-api-exposure))
...
| vLLM manager | [runtime-profiles.md](../../sdd/spec/runtime-profiles.md) | `packages/node-agent/internal/agent/vllm_manager.go::VllmManager` <!-- @impl: packages/node-agent/internal/agent/vllm_manager.go::VllmManager --> |
| vLLM install | [node-agent.md](../../sdd/spec/node-agent.md) | `packages/node-agent/internal/agent/vllm_install.go::EnsureVllm` <!-- @impl: packages/node-agent/internal/agent/vllm_install.go::EnsureVllm --> |
...
| Router behavioral tests | [router-worker.md](../../sdd/spec/router-worker.md) | `packages/router-worker/src/router-routing.test.ts::RouteFamilySeparationTestAnchor` <!-- @impl: packages/router-worker/src/router-routing.test.ts::RouteFamilySeparationTestAnchor --> |
| Agent behavioral tests | [node-agent.md](../../sdd/spec/node-agent.md) | `packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE001ServiceSkeletonAndListenerPolicy` <!-- @impl: packages/node-agent/internal/agent/agent_node_test.go::TestREQNODE001ServiceSkeletonAndListenerPolicy --> |
