...
9. The Overview "Mesh status" panel renders a responsive grid of per-mesh dashboard cards, each edged in its status tone (green when serving, amber when a deployed model has no serving machine, neutral for a mesh with no model) and structured as: the mesh name in the mesh vocabulary color with the status word beside it, the callable route in mono, the active model's display name over its own row of provider and serving-mode pills (or a no-model note with no pills), a machines/serving/tok-s metric strip, and a serving-capacity track filled to the served fraction — all carried as `data-state`/`data-state-tone`/`data-machines`/`data-serving`/`data-toks`/`data-fill` contract attributes; the Overview carries no activity feed (the activity log lives in Settings). <!-- @impl: packages/router-worker/src/admin-ui-client.ts::renderMeshStatus --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::routeChipEl --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::profilePills --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 overview mesh status rows summarize each mesh: model, machines, serving, throughput) -->
