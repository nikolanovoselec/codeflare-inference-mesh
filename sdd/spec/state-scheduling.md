...
2. A node is ineligible when the requested profile is unsupported, the runtime is not ready, the loaded model does not match the active profile, the node is over capacity, the node is under failure penalty, or Mesh connection data is invalid. <!-- @impl: packages/router-worker/src/scheduler.ts::isEligible --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-SCH-003 REQ-OBS-004 excludes expired unhealthy and unsafe nodes from scheduling) -->
