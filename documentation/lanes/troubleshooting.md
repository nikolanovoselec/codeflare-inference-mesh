...
**Cause:** Two distinct issues collapse to this generic Gateway error. If Gateway logs show no provider step or `provider=unknown`, the caller token likely lacks `AI Gateway: Run`. If Gateway logs show `provider=custom-codeflare-inference-mesh`, `model=dynamic/codeflare-mesh`, and metadata such as `user`, the route reached the router but the direct llama.cpp profile needs a session identity. Gateway log metadata from the `cf-aig-metadata` header is not forwarded to the custom provider on the REST dynamic-route path; the router can derive affinity only from metadata that reaches the Worker, such as OpenAI body `metadata.user` or explicit `body.user`, after the metadata-affinity fix. ([REQ-SEC-012](../../sdd/spec/security.md), [REQ-SCH-004](../../sdd/spec/state-scheduling.md#req-sch-004-direct-session-affinity))

**Fix:** First confirm the client's Cloudflare token includes `AI Gateway: Run` and is used with `cf-aig-gateway-id`. Then ensure the Worker version includes metadata-to-affinity translation, and send either OpenAI `user: "user:<id>|session:<id>"` explicitly or body `metadata: { "user": "<id>", "session": "<optional-session>" }`. Confirm with a chat completion for `dynamic/codeflare-mesh` and inspect Gateway logs for `status_code=200`.
