...
7. The created route config forwards the fixed stable public model `codeflare-mesh` to the custom provider with 3 retries and a long (120s) request timeout for long-running local inference; if the Gateway request reaches the custom provider with `model: dynamic/codeflare-mesh`, the router normalizes that dynamic-route name back to `codeflare-mesh` before profile selection so MeshLLM routing does not depend on user metadata. <!-- @impl: packages/router-worker/src/cloudflare-api.ts::syncCustomProvider --> <!-- @impl: packages/router-worker/src/router.ts::routablePublicModel --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 uses idempotent Cloudflare custom-provider and dynamic-route payload contracts) --> <!-- @test: packages/router-worker/src/router.test.ts (REQ-GWY-003 REQ-RTR-002 accepts AI Gateway dynamic route model names for MeshLLM profiles) -->
