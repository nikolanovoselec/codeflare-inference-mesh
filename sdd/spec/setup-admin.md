...
9. The Overview "Mesh status" panel renders one row per machine group — the shared mesh identity header (name plus right-aligned callable route), the active model's display name (or a no-model note), machine and serving counts, live aggregate tok/s when serving — with a status word whose tone is green when serving, amber when a deployed model has no serving machine, and neutral for a mesh with no model; the Overview carries no activity feed (the activity log lives in Settings). <!-- @impl: packages/router-worker/src/admin-ui-client.ts::renderMeshStatus --> <!-- @impl: packages/router-worker/src/admin-ui-client.ts::meshRowHead --> <!-- @test: packages/router-worker/src/admin-ui-dashboard.test.ts (REQ-ADM-015 overview mesh status rows summarize each mesh: model, machines, serving, throughput) -->
