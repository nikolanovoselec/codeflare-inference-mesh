...
6. A transiently failed runtime switch never wedges the node: on every heartbeat the agent reconciles the running manager's runtime kind against the selected profile's runtime and, when they disagree (both being managed kinds — external mode and unknown runtimes never participate), relaunches through the cross-runtime switch path until actual matches desired. <!-- @impl: packages/node-agent/cmd/inference-mesh-agent/main.go::runtimeKindMismatch --> <!-- @test: packages/node-agent/cmd/inference-mesh-agent/main_test.go (TestREQRUN010RuntimeKindMismatchSelfHealsEachHeartbeat) -->
